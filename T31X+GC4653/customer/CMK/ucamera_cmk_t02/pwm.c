...
int SU_PWM_Modify4ChnDuty( uint32_t duty0,uint32_t duty1,uint32_t duty2,uint32_t duty3)
{
	int ret = 0;
	SUPWM *pwm = getpwm();
	if(pwm == NULL) {
	...
	pwm->attr[0]->duty = duty0;
	ret = ioctl(pwm->fd, PWM_CONFIG_DUTY, (unsigned long)pwm->attr[0]);
	if(ret != 0) {
		printf("ioctl : %d error !\n", __LINE__);
		return -1;
	}
		pwm->attr[1]->duty = duty1;
	ret = ioctl(pwm->fd, PWM_CONFIG_DUTY, (unsigned long)pwm->attr[1]);
	if(ret != 0) {
		printf("ioctl : %d error !\n", __LINE__);
		return -1;
	}
			pwm->attr[2]->duty = duty2;
	ret = ioctl(pwm->fd, PWM_CONFIG_DUTY, (unsigned long)pwm->attr[2]);
	if(ret != 0) {
		printf("ioctl : %d error !\n", __LINE__);
		return -1;
	}
	pwm->attr[3]->duty = duty3;
	ret = ioctl(pwm->fd, PWM_CONFIG_DUTY, (unsigned long)pwm->attr[3]);
	if(ret != 0) {
		printf("ioctl : %d error !\n", __LINE__);
		return -1;
	}
