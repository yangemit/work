...
{
	/*modify fot tcl protcol*/
	//printk("composite_setup entry bRequestType=%02x.bRequest=%02x wValue=%04x wIndex=%04x,wLength=%d \n",ctrl->bRequestType, ctrl->bRequest,ctrl->wValue, ctrl->wIndex, ctrl->wLength);

	if(ctrl->bRequestType==0xa0){
			printk("composite_setup ctrl->bRequestType=%x,length=%d \n",ctrl->bRequestType,ctrl->wLength);
			ctrl->bRequestType=0xa1;
			ctrl->wIndex=0x600;
	}
	if(ctrl->bRequestType==0x20){
		printk("composite_setup ctrl->bRequestType=%x ,length=%d \n",ctrl->bRequestType,ctrl->wLength);
