...
//#define CONFIG_USB_DWC2_VERBOSE_VERBOSE
...
static void dwc2_gadget_set_in_nak(struct dwc2 *dwc, int epnum) {
	...
	do
	{
		...
		if (timeout == 0) {
			...
			if(time_sec!=tstart.tv_sec)
			{
				time_sec= tstart.tv_sec;
				failed_cnt=0;
			}
			else{  /*failed count per second*/
			failed_cnt++;
			if(failed_cnt>3)
				{
				kernel_restart(NULL);
	...
	if (req->request.status == -EINPROGRESS) {
		...
		if (r->short_not_ok && (r->actual < r->length) && (status == 0)) {
			/*DWC2_GADGET_DEBUG_MSG("trans complete success but short is not ok! "
					...
					*/
					printk("trans complete success but short is not ok! "
					"req = 0x%p, actual = %d, len = %d\n",
					req, r->actual, r->length);
			status = -EINVAL;
		}
...
//printk("__dwc2_gadget_ep_set_halt\n");
	return 0;
}
...
static int dwc2_gadget_ep_set_halt(struct usb_ep *ep, int value) {
	...
	unsigned long	 flags;
	int		 ret;
//printk("dwc2_gadget_ep_set_halt\n");
