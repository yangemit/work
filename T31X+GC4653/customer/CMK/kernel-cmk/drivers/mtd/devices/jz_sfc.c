...
#define JZ_NOR_REG   \
	...
	JZ_NOR_INIT(SPINOR_OP_WRSR_EN,0,0,0,0,0,0), \
	...
	JZ_NOR_INIT(SPINOR_OP_READ_SECURITY,3,1,0,0,0,0),\
...
static int jz_sfc_pio_txrx(struct jz_sfc *flash, struct sfc_transfer *t)
{
	...
	if(flash->tx){
		...
		if ( flash->nor_info->addr_len != 0) {
			...
			else if ( flash->addr_len == 1) {
				flash->nor_info->addr_len = flash->addr_len;
				flash->addr = ((*(unsigned char*)(t->tx_buf + 1)) &0x000000ff);
			}
			else{
...
static int jz_spi_norflash_wr_status(struct jz_sfc *flash, int status,int num)
{
	int ret;
	unsigned char command_stage1[2];
	unsigned char command_stage2[2];

	struct sfc_transfer transfer[1];
	unsigned char cmd_wdsr[3] = {0x01, 0x31, 0x11};

	command_stage1[0] = cmd_wdsr[num];
	command_stage1[1] = status&0xff;

	command_stage2[0] =status&0xff;
...
static int jz_spi_norflash_write_sr_enable(struct jz_sfc *flash)
{
	int ret;
	unsigned char command_stage1[1];
	struct sfc_transfer transfer[1];

	command_stage1[0] = SPINOR_OP_WRSR_EN;
...
static int spi_norflash_lock(struct mtd_info *mtd, loff_t ofs, uint64_t len)
{
	struct jz_sfc *flash= to_jz_spi_norflash(mtd);
	...
	jz_spi_norflash_status(flash,&status_old,0);
//	printk("lock read status 0=%x\n",status_old);	
	status_new = status_old |0x1c;

	//jz_spi_norflash_status(flash,&status_old,1);
	//printk("lock read status 1=%x\n",status_old);
	jz_spi_norflash_write_sr_enable(flash);

	/* Only modify protection if it will not unlock other areas */
		if (jz_spi_norflash_wr_status(flash, status_new,0) < 0) {
				res = 1;
							mutex_unlock(&flash->lock);

				return res;
		}
		 jz_spi_norflash_wait_till_ready(flash);
//	jz_spi_norflash_status(flash,&status_old,0);
//	printk("lock new status %x\n",status_old);
	mutex_unlock(&flash->lock);

	return res;
}

static int spi_norflash_unlock(struct mtd_info *mtd, loff_t ofs, uint64_t len)
{
	struct jz_sfc *flash= to_jz_spi_norflash(mtd);
	int status_old, status_new;
	int res = 0;
	...
	/* Wait until finished previous command */
		res = jz_spi_norflash_wait_till_ready(flash);
		if (res) {
			printk("%s---%s---%d\n", __FILE__, __func__, __LINE__);
						mutex_unlock(&flash->lock);
	   ...
	   jz_spi_norflash_status(flash,&status_old,0);
	//	printk("unlock read status %x\n",status_old);
		status_new = status_old & 0xe3;

	//	jz_spi_norflash_status(flash,&status_old,1);
	//	printk("unlock read status 1=%x\n",status_old);
		jz_spi_norflash_write_sr_enable(flash);
		if (jz_spi_norflash_wr_status(flash, status_new,0) < 0) {
			res = 1;
						mutex_unlock(&flash->lock);
			...
			size_t *retlen, const unsigned char *buf)
{
	...
	flash = to_jz_spi_norflash(mtd);
	//spi_norflash_unlock(mtd, to, len);
...
#ifdef CONFIG_MTD_SPI_NOR_FLASH_OTP

static int spi_norflash_get_otp_info(struct mtd_info *mtd,
		struct otp_info *info, size_t len)
{
		printk("spi_norflash_get_otp_info!\n");
...
static int jz_spi_norflash_erase_otp(struct jz_sfc *flash, uint32_t offset)
{
	int ret = 0, j;
	int security_reg_addr;
	unsigned char command_stage1[5];
	struct sfc_transfer transfer[1];
	...
	command_stage1[0] = SPINOR_OP_ERASE_SECURITY;
   ...
   if(offset<1024)
   	{
		security_reg_addr=0x1000;
	 }
	 else if(offset<1024*2)
	 {
		security_reg_addr=0x2000;
	 }
	 else if(offset<1024*3)
	 {
		security_reg_addr=0x3000;
	 }
  /*printk("[Debug-%s-%d]--len = %d----\n",__func__,__LINE__,len);*/

	command_stage1[1]=(security_reg_addr>>16)&0xff;
	command_stage1[2]=(security_reg_addr>>8)&0xff;
	command_stage1[3]=(security_reg_addr>>0)&0xff;
	transfer[0].tx_buf  = command_stage1;
	transfer[0].tx_buf1 = NULL;
	transfer[0].rx_buf =  NULL;
	transfer[0].len = 0;//sizeof(command_stage1);
...
static int spi_norflash_read_otp(struct mtd_info *mtd,
		loff_t from, size_t len, size_t *retlen, u_char *buf)
{
	...
	struct sfc_transfer transfer[1];
	struct jz_sfc *flash;
	unsigned int security_reg_addr;
	unsigned int actual_len;
	unsigned int read_len,tmp_len;

	unsigned char *swap_buf = NULL;
	flash = to_jz_spi_norflash(mtd);
	swap_buf = flash->swap_buf;
	//printk("spi_norflash_read_otp addr[%x],len=%d\n",from,len);
	...
	mutex_lock(&flash->lock);
	command_stage1[0] = SPINOR_OP_READ_SECURITY;//SPINOR_OP_READ_FAST;
	/*
	command_stage1[1] = from >> 16;
	command_stage1[2] = from >> 8;
	command_stage1[3] = from;
   ...
   if(from<1024)
   	{
		security_reg_addr=from+0x1000;
	 }
	 else if(from<1024*2)
	 {
		security_reg_addr=from-1024+0x2000;
	 }
	 else if(from<1024*3)
	 {
		security_reg_addr=from-2048+0x3000;
	 }
  /*printk("[Debug-%s-%d]--len = %d----\n",__func__,__LINE__,len);*/
	...
	if(len+(security_reg_addr&0x3ff)< 1024)
		actual_len = len;
	else
		actual_len = 1024 - (security_reg_addr&0x3ff);


	transfer[0].tx_buf = command_stage1;
	transfer[0].tx_buf1 = NULL;
	transfer[0].rx_buf = swap_buf;
	transfer[0].len = actual_len+1;//add dummy data
  // for(i=0;i<4;i++)
	// 	printk("command_stage1[%d]=%x\n",i,command_stage1[i]);
	 
	 memset(swap_buf,0,(actual_len+1));
	ret = jz_sfc_pio_txrx(flash, transfer);
	//if(ret != transfer[0].len)
	//	dev_err(flash->dev,"the transfer length is error,%d,%s\n",__LINE__,__func__);
	...
	*retlen = actual_len;
	...
	memcpy(buf,&swap_buf[1],actual_len);
   //for(i=0;i<actual_len+1;i++)
 	//  printk("swap_buf[%d]=%x\n",i,swap_buf[i]);

	tmp_len=actual_len;
	for (i = actual_len; i < len; i += 1024) {
		
		command_stage1[0] = SPINOR_OP_READ_SECURITY;
		/*printk("[Debug-%s-%d]--len = %d----\n",__func__,__LINE__,len);*/
		security_reg_addr+=0x1000;
		command_stage1[1]=0x00;
		command_stage1[2]=(security_reg_addr>>8)&0xff;
		command_stage1[3]=0x00;
		...
		transfer[0].tx_buf1 = NULL;
		transfer[0].rx_buf = swap_buf;
		transfer[0].len = read_len+1;
	    memset(swap_buf,0,(read_len+1));

		ret = jz_sfc_pio_txrx(flash, transfer);
	  //  if(ret != transfer[0].len)
		//      dev_err(flash->dev,"the transfer length is error,%d,%s\n",__LINE__,__func__);

		memcpy(buf + tmp_len,&swap_buf[1],ret);
...
static int spi_norflash_write_otp(struct mtd_info *mtd,
		loff_t to, size_t len, size_t *retlen, u_char *buf)
{
    unsigned int i, ret, err;
	int  actual_len, write_len;
	unsigned char command_stage1[5];
	struct sfc_transfer transfer[1];
	unsigned char * swap_buf= NULL;
	struct jz_sfc *flash;
	...
	flash = to_jz_spi_norflash(mtd);
	//printk("spi_norflash_write_otp addr[%x],len=%d\n",to,len);
	if(len==0)
	{
		*retlen=0;
		return 0;
	...
	swap_buf = flash->swap_buf;
	mutex_lock(&flash->lock);
	jz_spi_norflash_erase_otp(mtd,to);
	...
	*retlen = 0;
	/* less than mtd->writesize */
		...
		command_stage1[0] = SPINOR_OP_WRITE_SECURITY;
	...
	if(to<1024)
   	{
		security_reg_addr=to+0x1000;
	 }
	 else if(to<1024*2)
	 {
		security_reg_addr=to-1024+0x2000;
	 }
	 else if(to<1024*3)
	 {
		security_reg_addr=to-2048+0x3000;
	 }
  /*printk("[Debug-%s-%d]--len = %d----\n",__func__,__LINE__,len);*/
...
{
	device->_get_fact_prot_info = spi_norflash_get_otp_info;
	device->_read_fact_prot_reg = spi_norflash_read_otp;
	device->_get_user_prot_info = spi_norflash_get_otp_info;
	device->_read_user_prot_reg = spi_norflash_read_otp;
	device->_write_user_prot_reg = spi_norflash_write_otp;
	printk("otp_setup !\n");
