		...
		const struct usb_ctrlrequest *ctrl)
{
	...
	/*DBG(cdev, "%s %d brequest:0x%x\n", __func__, __LINE__, ctrl->bRequest);*/
	/*printk("bRequest 0x%x, w_value 0x%04x, len %d, endpoint %d\n",*/
			/*ctrl->bRequest, w_value, len, ep);*/
...
//	pr_info("gaudio_write(%d)\n", count);
	...
	while (r > 0) {
		...
		if (req) {
			/*printk("####################3333333333333333333333333333333333333333333333333333333#########################\n");*/
			req_null_cnt = 0;
			len = r;
			...
			if (audio_is_req_empty(audio,&audio->audio_data_list) && audio->cur_req == NULL) {
				/*printk("####################4444444444444444444444444444444444444444444444444444444444444444#########################\n");*/
				audio->cur_req = req;
			...
			if (req_null_cnt > 5) {
				...
				printk("##########################11111111111111111111111##########req_null_cnt = %d########################\n", req_null_cnt);
			}
		}
		r -= len;
		offset += len;
...
static void audio_data_complete(struct usb_ep *ep, struct usb_request * req)
{
	...
	if(req)
	{
		/*printk("##################666666666666666666666666666666\n");*/
...
#ifdef CONFIG_GADGET_UAC1_CAP
	/* if (volume_is_set == 0) */
	/* 	mic_volume_control.data[UAC__CUR] = 0x0600; */
	mic_volume_control.data[UAC__MIN] = 0xFA00;
	mic_volume_control.data[UAC__MAX] = 0x0600;
	mic_volume_control.data[UAC__RES] = 0x0200;
#endif
	return 0;
}
