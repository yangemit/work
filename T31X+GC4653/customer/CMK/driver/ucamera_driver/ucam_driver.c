...
#define INCLUDE_FMASS_INTERFACE
...
#ifdef INCLUDE_FMASS_INTERFACE
#include "mass_storage.c"
#endif
...
static unsigned int UCAMERA_VENDOR_ID   =  0x05A3;	/* Linux Foundation */
static unsigned int UCAMERA_PRODUCT_ID  =  0x9331;	/* Ucam A/V gadget */
static unsigned int UCAMERA_DEVICE_BCD  =  0x0100;	/* 0.10 */


#define STRING_DESCRIPTION_IDX		USB_GADGET_FIRST_AVAIL_IDX

static char ucamera_vendor_label[64] = "Ingenic Semiconductor CO., LTD.";
static char ucamera_product_label[64] = "H264 USB Camera";
...
static unsigned int hid_start = 0;
...
int ucamera_bind_fmass(struct usb_configuration *c)
{
	int ret = 0;
...
ucamera_config_bind(struct usb_configuration *c)
{
	...
	ret = ucamera_bind_fmass(c);
	if(ret)
		printk("ucamera bind mass storage func failed\n");
...
int ucamera_product_msg_config(unsigned long arg)
{
	...
	memcpy(video_device_name, pcfg->product, 64);
	memcpy(audio_device_name, pcfg->product, 64);
//	strcpy(video_device_name, "NV50-HD220S");
//	strcpy(audio_device_name, "NV50-HD220S-MIC");
//	strcpy(video_device_name, "VN4-FHD200");
//	strcpy(audio_device_name, "VN4-FHD200-MIC");
