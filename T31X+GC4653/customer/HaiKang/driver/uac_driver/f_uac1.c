...
MODULE_PARM_DESC(mic_channel, "mic channel: 1 -- MODE_MONO, 2 --MODE_STEREO");

static int in_ep_actual_size = (MIC_SIMPLE_RATE * MIC_CH * 2 / 1000);
static int in_ep_req_count = (MIC_CH * 25);
...
int uac_set_descriptor(unsigned long arg)
{
	...
	in_ep_actual_size = (in_sample_rate * mic_channel * 2 / 1000);
	in_ep_req_count = (mic_channel * 25);
	/* printk("mic_channel = %d, in_sample_rate = %d \n", mic_channel, in_sample_rate); */
