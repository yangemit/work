...
static struct uvc_device *g_udev = NULL;
static struct Ucamera_Cfg *g_ucfg = NULL;
...
struct uvc_format_info uvc_formats_nv12 = {
...
static unsigned int g_nFormats = 3;
...
uvc_video_fill_buffer(struct uvc_device *dev, struct v4l2_buffer *buf)
{
	...
	switch (dev->fcc) {
		...
		case V4L2_PIX_FMT_NV12:
			len = v_func->get_Nv12Frame(dev->mem[buf->index].start);
			buf->bytesused = len > 0 ? len : dev->imgsize;
			/*printf("#############format is nv12############\n");*/
			/*Ucamera_LOG("UVC video get nv12 frame len:%d\n", len);*/
			break;
		case V4L2_PIX_FMT_MJPEG:
			...
			buf->bytesused = len > 0 ? len : dev->imgsize;
			if (len > dev->imgsize) {
				Ucamera_LOG("UVC: image size too big size =%d, len=%d!\n",dev->imgsize,len);
			}
...
uvc_handle_streamon_event(struct uvc_device *dev)
{
	...
	if (ret < 0){
		printf("uvc_video_reqbufs is failed\n");
		goto err;
	}
			   ...
			   uint8_t len, struct uvc_request_data *resp)
{
	...
	case UVC_EXTENSION_UNIT_ID:
		/*按照客户定的协议填充CUR，MAX，MIN，RES等值，下面只是一个例子*/
			...
			switch (req) {
				...
				case UVC_GET_DEF:
					if (euctl.get != NULL)
					  	value = euctl.get(cs, req, (void *)resp->data, len);
					resp->length = len;
	...
	case UVC_PROCESSING_UNIT_ID:
		...
		case UVC_PU_WHITE_BALANCE_TEMPERATURE_AUTO_CONTROL:
			...
			if(cs == UVC_PU_WHITE_BALANCE_TEMPERATURE_AUTO_CONTROL)
				printf("___________value=%d___________\n",value);
			if (puctl[cs-1].set != NULL)
...
uvc_events_process(struct uvc_device *dev)
{
	...
	case UVC_EVENT_STREAMON: {
		...
		u_frame.interval = dev->interval;
		/*uvc_video_enum_format(dev->uvc_fd);*/
...
int gaudio_play_fifo_init(int fd)
{
	...
	udev->audio_base_addr_phy = (unsigned int)fifo_info.base;
 ...
 *
 */
static int set_yvyv_frames(struct Ucamera_Video_Cfg *vcfg, int index, struct uvc_frame_info **frame_info)
{
	int i = 0;
	int cnt = index;

	if (!vcfg || !frame_info || *frame_info == NULL) {
		Ucamera_ERR("set_yvyv_frames, invalid input params\n ");
		return -1;
	}

	struct uvc_frame_info *frame = *frame_info;

	while (i < vcfg->yuyvnum) {
		frame[cnt].width = vcfg->yuyvlist[i].width;
		frame[cnt].height = vcfg->yuyvlist[i].height;
		/*frame[cnt].intervals[0] = 10000000 / 25;*/
		if(frame[cnt].height >= 1440)
			frame[cnt].intervals[0] = 10000000 / 5;
		else if (frame[cnt].height >= 1080)
			frame[cnt].intervals[0] = 10000000 / 5;
		else if (frame[cnt].height >= 960)
			frame[cnt].intervals[0] = 10000000 / 5;
		else if (frame[cnt].height >= 720)
			frame[cnt].intervals[0] = 10000000 / 10;
		else
 ...
 *
 */
static int set_mjpeg_frames(struct Ucamera_Video_Cfg *vcfg, int index, struct uvc_frame_info **frame_info)
{
	int i = 0;
	int cnt = index;
	if (!vcfg || !frame_info || *frame_info == NULL) {
		Ucamera_ERR("set_mjpeg_frames, invalid input params\n ");
		return -1;
	}

	struct uvc_frame_info *frame = *frame_info;
	int support_frames = UVC_SUPPORT_2_FRAMES;

	while (i < vcfg->jpegnum) {
		frame[cnt].width = vcfg->jpeglist[i].width;
		frame[cnt].height = vcfg->jpeglist[i].height;
#ifdef HIK_2K_CAMERA
		if (vcfg->jpeglist[i].width <= 1080) {
			support_frames = UVC_SUPPORT_4_FRAMES;
		} else {
			support_frames = UVC_SUPPORT_2_FRAMES;
		}
#endif

		switch (support_frames)
		{
		case UVC_SUPPORT_4_FRAMES:
			frame[cnt].intervals[0] = 10000000 / 60;
			frame[cnt].intervals[1] = 10000000 / 50;
			frame[cnt].intervals[2] = 10000000 / 30;
			frame[cnt].intervals[3] = 10000000 / 25;
			break;
		case UVC_SUPPORT_2_FRAMES:
			frame[cnt].intervals[0] = 10000000 / 30;
 ...
 *
 */
static int set_h264_frames(struct Ucamera_Video_Cfg *vcfg, int index, struct uvc_frame_info **frame_info)
{
	int i = 0;
	int cnt = index;
	if (!vcfg || !frame_info || *frame_info == NULL) {
		Ucamera_ERR("set_h264_frames, invalid input params\n ");
		return -1;
	}

	struct uvc_frame_info *frame = *frame_info;
	int support_frames = UVC_SUPPORT_1_FRAMES;
 ...
 *
 */
static int set_nv12_frames(struct Ucamera_Video_Cfg *vcfg, int index, struct uvc_frame_info **frame_info)
{
	int i = 0;
	int cnt = index;

	if (!vcfg || !frame_info || *frame_info == NULL) {
		Ucamera_ERR("set_nv12_frames, invalid input params\n ");
		return -1;
	}
	...
	while (i < vcfg->yuyvnum) {
		...
		if(frame[cnt].height >= 1440)
			...
			frame[cnt].intervals[0] = 10000000 / 15;
		else
			frame[cnt].intervals[0] = 10000000 / 30;
		cnt++;
		i++;
...
int Ucamera_Config(struct Ucamera_Cfg *ucfg)
{
	...
	vcfg = &(ucfg->vcfg);
	/*
	 *nframes = vcfg->h264num + vcfg->jpegnum + vcfg->yuyvnum * 2;
	 */
	...
	if (ucfg->h264_en == 1) {
		...
		cnt = set_h264_frames(vcfg, cnt, &frame);
	} else {
		uvc_formats_nv12.frames = &frame[cnt];
		uvc_formats_nv12.nframes = vcfg->yuyvnum;
		cnt = set_nv12_frames(vcfg, cnt, &frame);
	}

	if (ucfg->h264_en) {
		uvc_formats[0] = &uvc_formats_h264;
		uvc_formats[1] = &uvc_formats_yuyv;
		uvc_formats[2] = &uvc_formats_mjpeg;
	} else {
		uvc_formats[0] = &uvc_formats_yuyv;
		uvc_formats[1] = &uvc_formats_mjpeg;
		uvc_formats[2] = &uvc_formats_nv12;
	}
...
int Ucamera_Video_Regesit_Camera_Terminal_CB(struct Ucamera_Video_CT_Control *ctctl[])
{
	...
	for (i = 0; (ctctl[i] != NULL) && (i < UVC_PRIVACY_CONTROL); i++) {
		...
		case UVC_ROLL_ABSOLUTE_CONTROL:
		case UVC_PRIVACY_CONTROL:
...
int Ucamera_Video_Regesit_Process_Unit_CB(struct Ucamera_Video_PU_Control *puctl[])
{
	...
	for (i = 0; (puctl[i] != NULL) && (i < UVC_WHITE_BALANCE_TEMPERATURE_AUTO_CONTROL + 1); i++) {
		ictl = puctl[i];
		type = ictl->type;
		switch (type)
		{
