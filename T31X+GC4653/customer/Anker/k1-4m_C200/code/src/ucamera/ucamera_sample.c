/*
 * sample-Encoder-jpeg.c
 *
 * Copyright (C) 2014 Ingenic Semiconductor Co.,Ltd
 */

#include <pthread.h>
#include <fcntl.h>
#include <stdio.h>
#include <stdlib.h>
#include <signal.h>
#include <string.h>
#include <math.h>
#include <time.h>
#include <semaphore.h>
#include <sys/ioctl.h>
#include <sys/time.h>

#include <imp/imp_log.h>
#include <imp/imp_common.h>
#include <imp/imp_system.h>
#include <imp/imp_framesource.h>
#include <imp/imp_encoder.h>
#include <usbcamera.h>
#include <AntiCopy_Verify.h>
#include <imp-common.h>
#include <camera_on_off.h>
#include <af_control.h>
#include <cdc_log_info.h>

#define TAG "Sample-UCamera"
#define LABEL_LEN                64
#define GET_CUR			0x81
#define GET_LEN			0x85

#define PI 			3.1415926

#ifndef T20
static int af_on_off = 0;
static int focus_trigger_value = 30;
static int motor_pixel = 200;
static int motor_step = 25;
static int motor_sleep_time = 80 * 1000;
static int motor_step_max = 400;
static int motor_step_min = 300;

static int ev_last;
static int ev_value = 0;

#endif

static char manufact_label[LABEL_LEN] = "Anker PowerConf C200";
static char product_label[LABEL_LEN] = "Anker PowerConf C200";
static char serial_label[LABEL_LEN] = "1234567890123456";
static char video_device_name[LABEL_LEN] = "Anker PowerConf C200";
static char audio_device_name[LABEL_LEN] = "Anker PowerConf C200";

static char config_version[LABEL_LEN] = "v0.9";
static int  VENDOR_ID = 0x291a;
static int  PRODUCT_ID = 0x3369;
static int  DEVICE_BCD = 0x0105;

static int spk_volume_line = 0;

static int Ucam_Stream_On = 0;
extern struct chn_conf chn[];
#ifdef T31
static crop_obj_t fov_crop;
static crop_obj_t zoom_crop;
static int pantilt_mode = FOV_MODE;
static float  pan_factor;
static float  tilt_factor;
#endif
static struct Ucamera_Zoom_Cfg zoom_cfg;

FILE *uvc_attr_fd = NULL;
FILE *uvc_config_fd = NULL;
pthread_mutex_t uvc_attr_mutex;
sem_t ucam_ready_sem;
#ifndef T20
static int imp_inited = 0;
#endif
IMPISPGamma gamma_cur;
static int chn_attr_changed = 0;
static uint8_t first_set_flag = 0;
static int led_level = 1;
static struct Ucamera_Video_Frame *pframe = NULL;
static struct timeval time_start;

struct uvc_pu_string {
	char id;
	const char *s;
};
struct uvc_ct_string {
	char id;
	const char *s;
};

typedef struct _uvc_eu_attr {
	int fov;
	int audio_angle;
	uint64_t device_mode_set;
	int hdr_mode;
} uvc_eu_attr_t;

static uvc_eu_attr_t uvc_eu_attr = {
	.fov = SENSOR_FOV_95,
	.audio_angle = AUDIO_ANGLE_90,
	.device_mode_set = 0,
	.hdr_mode = 0,
};

/*static long time_sum = 0;*/

struct uvc_pu_string pu_string[] = {
	{0, "null"},
	{UVC_BACKLIGHT_COMPENSATION_CONTROL, "backlight"},
	{UVC_BRIGHTNESS_CONTROL, "brightness"},
	{UVC_CONTRAST_CONTROL, "contrast"},
	{UVC_GAIN_CONTROL, "gain"},
	{UVC_POWER_LINE_FREQUENCY_CONTROL, "powerline_freq"},
	{UVC_HUE_CONTROL, "hue"},
	{UVC_SATURATION_CONTROL, "saturation"},
	{UVC_SHARPNESS_CONTROL, "sharpness"},
	{UVC_GAMMA_CONTROL, "gamma"},
	{UVC_WHITE_BALANCE_TEMPERATURE_CONTROL, "white_balance"},
	{UVC_WHITE_BALANCE_TEMPERATURE_AUTO_CONTROL, "white_balance_auto"},
	{UVC_HVFLIP_CONTROL, "hvflip"}
};

struct uvc_ct_string ct_string[] = {
	{0, "null"},
	{UVC_PAN_ATTR_CONTROL, "pan"},
	{UVC_TILT_ATTR_CONTROL, "tilt"},
	{EU_FOV_ATTR_CONTROL, "fov"},
	{EU_AUDIO_ANGLE_ATTR_CONTROL, "audio_angle"},
	{EU_PANTILT_MODE_CONTROL, "pantilt_mode"},
	{EU_HDR_MODE_CONTROL, "hdr_mode"},
	{7, "null"},
	{8, "null"},
	{9, "null"},
	{10, "null"},
	{UVC_ZOOM_ABSOLUTE_CONTROL, "zoom"},
	{12, "null"},
};

static struct Ucamera_Video_CT_Control auto_exposure_mode = {
	.type = UVC_AUTO_EXPOSURE_MODE_CONTROL,
	.data[UVC_MIN] = 1,
	.data[UVC_MAX] = 4,
	.data[UVC_DEF] = 2,
	.data[UVC_CUR] = 2,
};

static struct Ucamera_Video_CT_Control exposure_time = {
	.type = UVC_EXPOSURE_TIME_CONTROL,
	.data[UVC_MIN] = 1,
	.data[UVC_MAX] = 1000,
	.data[UVC_DEF] = 156,
	.data[UVC_CUR] = 156,
};

static struct Ucamera_Video_CT_Control focus = {
	.type = UVC_FOCUS_CONTROL,
	.data[UVC_MIN] = 300,
	.data[UVC_MAX] = 400,
	.data[UVC_DEF] = 300,
	.data[UVC_CUR] = 300,
};

static struct Ucamera_Video_CT_Control focus_auto = {
    .type = UVC_FOCUS_AUTO_CONTROL,
    .data[UVC_MAX] = 1,
    .data[UVC_MIN] = 0,
    .data[UVC_DEF] = 1,
    .data[UVC_CUR] = 1,
};

static struct Ucamera_Video_CT_Control zoom = {
	.type = UVC_ZOOM_ABSOLUTE_CONTROL,
	.data[UVC_MAX] = 400,
	.data[UVC_MIN] = 100,
	.data[UVC_DEF] = 100,
	.data[UVC_CUR] = 100,
};

static struct Ucamera_Video_CT_Control pantilt = {
	.type = UVC_PANTILT_ABSOLUTE_CONTROL,
	.ptdata[UVC_MAX] = 0x8ca000008ca0,
	.ptdata[UVC_MIN] = 0xffff7360ffff7360,
	.ptdata[UVC_DEF] = 0,
	.ptdata[UVC_CUR] = 0,
};

static struct Ucamera_Video_PU_Control backlight_compens = {
	.type = UVC_BACKLIGHT_COMPENSATION_CONTROL,
	.data[UVC_MIN] = 0,
	.data[UVC_MAX] = 4,
	.data[UVC_DEF] = 0,
	.data[UVC_CUR] = 0,
};

static struct Ucamera_Video_CT_Control *Ct_ctrl[] = {
    &exposure_time,
    &auto_exposure_mode,
    &focus,
    &focus_auto,
    &zoom,
    &pantilt,
    NULL,
};

static struct Ucamera_Video_PU_Control brightness = {
	.type = UVC_BRIGHTNESS_CONTROL,
	.data[UVC_MIN] = 0,
#if 0
	.data[UVC_MAX] = 255,
	.data[UVC_DEF] = 128,
	.data[UVC_CUR] = 128,
#endif
	.data[UVC_MAX] = 100,
	.data[UVC_DEF] = 50,
	.data[UVC_CUR] = 50,
};

static struct Ucamera_Video_PU_Control contrast = {
	.type = UVC_CONTRAST_CONTROL,
	.data[UVC_MIN] = 0,
#if 0
	.data[UVC_MAX] = 200,
	.data[UVC_DEF] = 128,
	.data[UVC_CUR] = 128,
#endif
	.data[UVC_MAX] = 100,
	.data[UVC_DEF] = 50,
	.data[UVC_CUR] = 50,
};

static struct Ucamera_Video_PU_Control saturation = {
	.type = UVC_SATURATION_CONTROL,
	.data[UVC_MIN] = 0,
#if 0
	.data[UVC_MAX] = 255,
	.data[UVC_DEF] = 128,
	.data[UVC_CUR] = 128,
#endif
	.data[UVC_MAX] = 100,
	.data[UVC_DEF] = 50,
	.data[UVC_CUR] = 50,
};

static struct Ucamera_Video_PU_Control sharpness = {
	.type = UVC_SHARPNESS_CONTROL,
	.data[UVC_MIN] = 0,
#if 0
	.data[UVC_MAX] = 200,
	.data[UVC_DEF] = 128,
	.data[UVC_CUR] = 128,
#endif
	.data[UVC_MAX] = 100,
	.data[UVC_DEF] = 50,
	.data[UVC_CUR] = 50,
};

static struct Ucamera_Video_PU_Control hue = {
	.type = UVC_HUE_CONTROL,
	.data[UVC_MIN] = 0,
	.data[UVC_MAX] = 255,
	.data[UVC_DEF] = 128,
	.data[UVC_CUR] = 128,
};

static struct Ucamera_Video_PU_Control hvflip = {
	.type = UVC_HUE_CONTROL,
	.data[UVC_MIN] = 0,
	.data[UVC_MAX] = 3,
	.data[UVC_DEF] = 1,
	.data[UVC_CUR] = 0,
};

static struct Ucamera_Video_PU_Control gamma_pu = {
	.type = UVC_GAMMA_CONTROL,
	.data[UVC_MIN] = 0,
	.data[UVC_MAX] = 800,
	.data[UVC_DEF] = 400,
	.data[UVC_CUR] = 400,
};


static struct Ucamera_Video_PU_Control whitebalance = {
	.type = UVC_WHITE_BALANCE_TEMPERATURE_CONTROL,
	.data[UVC_MIN] = 300,
	.data[UVC_MAX] = 600,
	.data[UVC_DEF] = 417,
};

static struct Ucamera_Video_PU_Control powerlinefreq = {
	.type = UVC_POWER_LINE_FREQUENCY_CONTROL,
	.data[UVC_MIN] = 0,
	.data[UVC_MAX] = 2,
	.data[UVC_DEF] = 0,
};

static struct Ucamera_Video_PU_Control *Pu_ctrl[] = {
	&brightness,
	&contrast,
	&hue,
	&hvflip,
	&saturation,
	&sharpness,
	&gamma_pu,
	&powerlinefreq,
	NULL,
};

enum screen_move_direction {
	SCREEN_UP,
	SCREEN_DOWM,
	SCREEN_LETF,
	SCREEN_RIGRH,
	SCREEN_NUEM
};
static int ucamera_hdr_change(int cmd);
static int strToInt(char const* str) {
	int val;
	if (sscanf(str, "%d", &val) == 1) return val;
	return -1;
}

static int strToInt_Hex(char const* str) {
	int val;
	if (sscanf(str, "%x", &val) == 1) return val;
	return -1;
}

static int Dec_to_Hex(int const dec) {
	char str[10] = {0};
	int val = 0;
	sprintf(str, "%x", dec);
	if (sscanf(str, "%x", &val) == 1) return val;
	return -1;
}

int htoi(char s[])
{
	int n = 0;
	int i = 0;
	while (s[i] != '\0' && s[i] != '\n') {
		if (s[i] == '0') {
			if (s[i+1] == 'x' || s[i+1] == 'X')
				i+=2;
		}
		if (s[i] >= '0' && s[i] <= '9') {
			n = n * 16 + (s[i] - '0');
		} else if (s[i] >= 'a' && s[i] <= 'f') {
			n = n * 16 + (s[i] - 'a') + 10;
		} else if (s[i] >= 'A' && s[i] <= 'F') {
			n = n * 16 + (s[i] - 'A') + 10;
		} else
			return -1;
		++i;
	}

	return n;
}

int sample_ucamera_set_fov(int angle)
{
	int ret;
	int left = 0 , top = 0 ,crop_width = 0 , crop_height = 0;
	IMPFSChnAttr *imp_chn_attr_set_fov;
	imp_chn_attr_set_fov = &chn[0].fs_chn_attr;
	IMPISPFrontCrop fcrop;

	/*
	* w = 2 * (g_VideoHeight / 2) * ctan(FOV/2) * tan(angle /2)
	*/

	if (angle == 0 && angle > SENSOR_DEFAULT_FOV) {
		Ucamera_ERR(" set fov (%d) is invalid \n", angle);
		CDC_LOG(UVC_SET_FOV_ERROR," set fov (%d) is invalid", angle);
		return 0;
	}

	crop_width =round(2.0* ((g_VideoWidth / 2)/ tan((SENSOR_DEFAULT_FOV / 2) * PI / 180)) * tan((angle / 2) * PI / 180 ));
	left = (g_VideoWidth - crop_width) / 2;
	crop_height =round(1.0 * g_VideoHeight * crop_width / g_VideoWidth);
	top = (g_VideoHeight - crop_height) / 2;

	if(zoom_cfg.height == g_VideoHeight && angle < SENSOR_FOV_65 ){
		ret = sample_framesource_streamoff();
		if(ret<0){
			IMP_LOG_ERR(TAG, "ImpStreamOn failed\n");
			return -1;
		}

		if(crop_width % 16){
			crop_width -= crop_width % 16;
		}

		if(crop_height % 16){
			crop_height -= crop_height % 16;
		}

		imp_chn_attr_set_fov->scaler.enable = 0;
		imp_chn_attr_set_fov->crop.enable = 1;
		imp_chn_attr_set_fov->crop.left = left;
		imp_chn_attr_set_fov->crop.top = top;
		imp_chn_attr_set_fov->crop.width = crop_width;
		imp_chn_attr_set_fov->crop.height = crop_height;
		imp_chn_attr_set_fov->picWidth = crop_width;
		imp_chn_attr_set_fov->picHeight = crop_height;
		chn_attr_changed = 1;

		ret = IMP_FrameSource_SetChnAttr(0,imp_chn_attr_set_fov);
		if(ret < 0){
			IMP_LOG_ERR(TAG,"%s(%d):IMP_FrameSource_SetChnAttr failed",__func__,__LINE__);
			return -1;
		}

		ret = sample_framesource_streamon();
		if(ret<0){
			IMP_LOG_ERR(TAG, "ImpStreamOff failed\n");
			return -1;
		}

	} else {
		if(chn_attr_changed) {
			ret = sample_framesource_streamoff();
			if(ret<0){
				IMP_LOG_ERR(TAG, "ImpStreamOn failed\n");
				return -1;
			}

			if(crop_width % 16){
				crop_width -= crop_width % 16;
			}
			if(crop_height % 16){
				crop_height -= crop_height % 16;
			}
			imp_chn_attr_set_fov->scaler.enable = 1;
			imp_chn_attr_set_fov->crop.enable = 0;
			/*imp_chn_attr_set_fov->crop.left = left;
			imp_chn_attr_set_fov->crop.top = top;
			imp_chn_attr_set_fov->crop.width = zoom_cfg.width;
			imp_chn_attr_set_fov->crop.height = zoom_cfg.height;*/
			imp_chn_attr_set_fov->picWidth = zoom_cfg.width;
			imp_chn_attr_set_fov->picHeight = zoom_cfg.height;
			ret = IMP_FrameSource_SetChnAttr(0,imp_chn_attr_set_fov);
			if(ret < 0){
				IMP_LOG_ERR(TAG,"%s(%d):IMP_FrameSource_SetChnAttr failed",__func__,__LINE__);
				return -1;
			}

			ret = sample_framesource_streamon();
			if(ret<0){
				IMP_LOG_ERR(TAG, "ImpStreamOff failed\n");
				return -1;
			}
			chn_attr_changed = 0;
		}

		if (crop_width % 2)
			crop_width -= crop_width % 2;
		if (crop_height % 2)
			crop_height -= crop_height % 2;

		memset(&fcrop, 0, sizeof(IMPISPFrontCrop));

		fcrop.fcrop_enable = 1;
		fcrop.fcrop_left = left;
		fcrop.fcrop_top = top;
		fcrop.fcrop_width = crop_width;
		fcrop.fcrop_height = crop_height;

		ret = IMP_ISP_Tuning_SetFrontCrop(&fcrop);
		if (ret < 0) {
			Ucamera_LOG("IMP Set Fcrop failed=%d\n",__LINE__);
			CDC_LOG(UVC_SET_FOV_ERROR," set fov (%d) error", angle);
			return -1;
		}
	}

	/*TODO: */
	/* save left top w h */
	fov_crop.left = left;
	fov_crop.top = top;
	fov_crop.w = crop_width;
	fov_crop.h = crop_height;

	ucamera_uvc_ct_attr_save(EU_FOV_ATTR_CONTROL, angle);
	uvc_eu_attr.fov = angle;

//	printf("FOV_VALUE value = %d\n",angle);
	/*printf("zoom_cfg.height is %d",zoom_cfg.height);*/
	/*printf("left is %d,top is %d,width is %d,height is %d\n",fov_crop.left,fov_crop.top,fov_crop.w,fov_crop.h);*/

	return 0;
}

int sample_ucamera_set_audio_angle(int angle)
{
	/*TODO: audio algo api */
	/* save */
	ucamera_uvc_ct_attr_save(EU_AUDIO_ANGLE_ATTR_CONTROL, angle);
	return 0;
}

int sample_video_pantilt_set(void *ptvalue)
{
	int ret = 0;
	uint8_t value[8] = {0};
	memcpy(value, ptvalue, sizeof(uint8_t) * 8);
	IMPFSChnAttr *imp_chn_attr_set_pantilt;
	imp_chn_attr_set_pantilt = &chn[0].fs_chn_attr;

	/*filter first set caused by enumeration*/
	uint64_t  timeuse = 0;
	struct timeval time_end;
	gettimeofday(&time_end, NULL);
	timeuse = (uint64_t)(1000000 * (time_end.tv_sec - time_start.tv_sec)) + time_end.tv_usec - time_start.tv_usec;
	if(timeuse < 1000000){
		return 0;
	}

	/*0xffffffff */
	int pan = value[3] << 24 | value[2] << 16 | value[1] << 8 | value[0];        /* LOW 4 byte */
	int tilt = value[7] << 24 | value[6] << 16 | value[5] << 8 | value [4];      /* Hei 4 byte */

	/*changel direction */
	int pan_set = 0, tilt_set = 0;
	tilt_set = -tilt;

	if (hvflip.data[UVC_CUR] == 1) {
		pan_set = -pan;
	} else {
		pan_set = pan;
	}

	int left = 0;
	int top = 0;
	int w = zoom_crop.w;
	int h =zoom_crop.h;

	pan_factor = 1.0 * pan_set / 36000;
	tilt_factor = 1.0 * tilt_set / 36000;

	left = round(1.0 *(1.0 * (g_VideoWidth - w) / 2) * (1 + 1.0 * pan_set / 36000));
	top = round(1.0 * (1.0 * (g_VideoHeight - h) / 2) * (1 + 1.0 * tilt_set / 36000));

	if (pan_set == 0 && tilt_set == 0) {
		/*w = g_VideoWidth;*/
		/*h = g_VideoHeight;*/
		/*left = 0;*/
		/*top = 0;*/
		left = zoom_crop.left;
		top = zoom_crop.top;
	}
#if 1
	if(zoom_cfg.height== 1440 && zoom.data[UVC_CUR] > 400 && w != 0){
		ret = sample_framesource_streamoff();
		if(ret<0){
			IMP_LOG_ERR(TAG, "ImpStreamOn failed\n");
			return -1;
		}

		if(w % 16){
			w -= w % 16;
		}
		if(h % 16){
			h -= h % 16;
		}
		imp_chn_attr_set_pantilt->scaler.enable = 0;
		imp_chn_attr_set_pantilt->crop.enable = 1;
		imp_chn_attr_set_pantilt->crop.left = left;
		imp_chn_attr_set_pantilt->crop.top = top;
		imp_chn_attr_set_pantilt->crop.width = w;
		imp_chn_attr_set_pantilt->crop.height = h;
		imp_chn_attr_set_pantilt->picWidth = w;
		imp_chn_attr_set_pantilt->picHeight = h;
		chn_attr_changed = 1;

		printf("2k is set pantilt\n");

		printf("w = %d, h = %d, left = %d, top =%d \n", w, h, left, top);
		ret = IMP_FrameSource_SetChnAttr(0,imp_chn_attr_set_pantilt);
		if(ret < 0){
			IMP_LOG_ERR(TAG,"%s(%d):IMP_FrameSource_SetChnAttr failed",__func__,__LINE__);
			return -1;
		}

		ret = sample_framesource_streamon();
		if(ret<0){
			IMP_LOG_ERR(TAG, "ImpStreamOff failed\n");
			return -1;
		}


	}else if (w != 0){

		if(chn_attr_changed){
			ret = sample_framesource_streamoff();
			if(ret<0){
				IMP_LOG_ERR(TAG, "ImpStreamOff failed\n");
				return -1;
			}
			imp_chn_attr_set_pantilt->scaler.enable = 1;
			imp_chn_attr_set_pantilt->crop.enable = 0;
			/*imp_chn_attr_set_pantilt->crop.left = left;
			imp_chn_attr_set_pantilt->crop.top = top;
			imp_chn_attr_set_pantilt->crop.width = zoom_cfg.width;
			imp_chn_attr_set_pantilt->crop.height = zoom_cfg.height;*/
			imp_chn_attr_set_pantilt->picWidth = zoom_cfg.width;
			imp_chn_attr_set_pantilt->picHeight = zoom_cfg.height;
			ret = IMP_FrameSource_SetChnAttr(0,imp_chn_attr_set_pantilt);
			if(ret < 0){
				IMP_LOG_ERR(TAG,"%s(%d):IMP_FrameSource_SetChnAttr failed",__func__,__LINE__);
				return -1;
			}

			ret = sample_framesource_streamon();
			if(ret<0){
				IMP_LOG_ERR(TAG, "ImpStreamOn failed\n");
				return -1;
			}
			chn_attr_changed = 0;
		}

		IMPISPFrontCrop crop_obj;
		crop_obj.fcrop_enable = 1;
		crop_obj.fcrop_left = left;
		crop_obj.fcrop_top = top;
		crop_obj.fcrop_width = w;
		crop_obj.fcrop_height = h;

		ret = IMP_ISP_Tuning_SetFrontCrop(&crop_obj);
		if (ret < 0) {
			Ucamera_LOG("IMP Set Fcrop failed=%d\n",__LINE__);
			CDC_LOG(UVC_SET_PANTILT_ERROR,"uvc set pantilt error");
			return -1;
		}
	}
#endif


	pantilt.ptdata[UVC_CUR] = *((uint64_t *)ptvalue);
	/*printf("PANTILT_DATA : value = %llu \n",pantilt.ptdata[UVC_CUR]);*/
	printf("PANTILT_DATA : pan value = %d \n",pan);
	printf("PANTILT_DATA : tilt value = %d \n",tilt);
	printf("PANTILT left is%d, top is%d,width is %d, height is %d \n",left,top,w,h);
	ucamera_uvc_ct_attr_save(UVC_PAN_ATTR_CONTROL, pan);
	ucamera_uvc_ct_attr_save(UVC_TILT_ATTR_CONTROL, tilt);
#if 0
	if (pan == 0 && tilt == 0) {
		zoom.data[UVC_CUR] = 100;
		ucamera_uvc_ct_attr_save(UVC_ZOOM_ABSOLUTE_CONTROL, zoom.data[UVC_CUR]);
	}
#endif
	return 0;
}

uint64_t sample_video_pantilt_get()
{
	/*printf("get pantilt is doing,the pantilt value is %llu\n",pantilt.ptdata[UVC_CUR]);*/
	return pantilt.ptdata[UVC_CUR];
}

int sample_video_focus_set(int value)
{
    focus.data[UVC_CUR] = value;
    return 0;
}

int sample_video_focus_get()
{
	return focus.data[UVC_CUR];
}

int sample_video_focus_auto_set(int value)
{
    focus_auto.data[UVC_CUR] = value;
    return 0;
}

int sample_video_focus_auto_get()
{
	return focus_auto.data[UVC_CUR];
}
#ifdef T31
int sample_video_exposure_time_set(int value)
{
    int ret, value_time=0;
    if(value > 900)
	    value_time = 255;
    else if(value > 600)
	    value_time = 220;
    else if(value > 300)
	    value_time = 190;
    else
	    value_time = value;

    ret = IMP_ISP_Tuning_SetAeComp(value_time);
    if(ret)
        Ucamera_LOG("ERROR:set exposure time Invalid leave:%d\n",value);

    exposure_time.data[UVC_CUR] = value;
    return 0;
}

int sample_video_exposure_time_get(void)
{
	return exposure_time.data[UVC_CUR];
}

int sample_video_auto_exposure_mode_set(int value)
{
    int ret;
    if(value == 2){
	    ret = IMP_ISP_Tuning_SetAeComp(128);
	    if(ret)
		    Ucamera_LOG("ERROR:set exposure time Invalid leave:%d\n",value);
    }
	auto_exposure_mode.data[UVC_CUR] = value;
	return 0;
}

int sample_video_auto_exposure_mode_get(void)
{
	return auto_exposure_mode.data[UVC_CUR];
}

int sample_video_zoom_set(int value)
{
	int ret;
	float value_cur;
	value_cur = value;
	int left, top, w, h;
	IMPFSChnAttr *imp_chn_attr_set_zoom;
	imp_chn_attr_set_zoom = &chn[0].fs_chn_attr;

	w = round((g_VideoWidth - 0 ) / sqrt(value_cur / 100));
	w = (round(1.0 * w / 4)) * 4;
	h = round((g_VideoHeight - 0 ) / sqrt(value_cur / 100));
	h = (round(1.0 * h / 4)) * 4;

	left = round(1.0 * (g_VideoWidth - w) / 2);
	top = round(1.0 * (g_VideoHeight - h) / 2);

	if(!first_set_flag){
		left = round(1.0 * left * (1 + pan_factor));
		top = round(1.0 * top * (1 + tilt_factor));
	}

#if 1
	if(zoom_cfg.height== 1440 && value > 400){
		ret = sample_framesource_streamoff();
		if(ret<0){
			IMP_LOG_ERR(TAG, "ImpStreamOn failed\n");
			return -1;
		}

		if(w % 16){
			w -= w % 16;
		}
		if(h % 16){
			h -= h % 16;
		}
		imp_chn_attr_set_zoom->scaler.enable = 0;
		imp_chn_attr_set_zoom->crop.enable = 1;
		imp_chn_attr_set_zoom->crop.left = left;
		imp_chn_attr_set_zoom->crop.top = top;
		imp_chn_attr_set_zoom->crop.width = w;
		imp_chn_attr_set_zoom->crop.height = h;
		imp_chn_attr_set_zoom->picWidth = w;
		imp_chn_attr_set_zoom->picHeight = h;
		chn_attr_changed = 1;

		printf("2k is set zoom\n");
		ret = IMP_FrameSource_SetChnAttr(0,imp_chn_attr_set_zoom);
		if(ret < 0){
			IMP_LOG_ERR(TAG,"%s(%d):IMP_FrameSource_SetChnAttr failed",__func__,__LINE__);
			return -1;
		}

		ret = sample_framesource_streamon();
		if(ret<0){
			IMP_LOG_ERR(TAG, "ImpStreamOff failed\n");
			return -1;
		}

	}else{

		if(chn_attr_changed){
			ret = sample_framesource_streamoff();
			if(ret<0){
				IMP_LOG_ERR(TAG, "ImpStreamOn failed\n");
				return -1;
			}
			imp_chn_attr_set_zoom->scaler.enable = 1;
			imp_chn_attr_set_zoom->crop.enable = 0;
			/*imp_chn_attr_set_zoom->crop.left = left;
			imp_chn_attr_set_zoom->crop.top = top;
			imp_chn_attr_set_zoom->crop.width = zoom_cfg.width;
			imp_chn_attr_set_zoom->crop.height = zoom_cfg.height;*/
			imp_chn_attr_set_zoom->picWidth = zoom_cfg.width;
			imp_chn_attr_set_zoom->picHeight = zoom_cfg.height;
			ret = IMP_FrameSource_SetChnAttr(0,imp_chn_attr_set_zoom);
			if(ret < 0){
				IMP_LOG_ERR(TAG,"%s(%d):IMP_FrameSource_SetChnAttr failed",__func__,__LINE__);
				return -1;
			}

			ret = sample_framesource_streamon();
			if(ret<0){
				IMP_LOG_ERR(TAG, "ImpStreamOff failed\n");
				return -1;
			}
			chn_attr_changed = 0;
		}

		IMPISPFrontCrop crop_obj;

		crop_obj.fcrop_enable = 1;
		crop_obj.fcrop_left = left;
		crop_obj.fcrop_top = top;
		crop_obj.fcrop_width = w;
		crop_obj.fcrop_height = h;

		if(pantilt_mode == FOV_MODE){
			goto save_crop_obj;
		}
		ret = IMP_ISP_Tuning_SetFrontCrop(&crop_obj);
		if (ret < 0) {
			Ucamera_LOG("IMP Set Fcrop failed=%d\n",__LINE__);
			CDC_LOG(UVC_SET_ZOOM_ERROR,"uvc set zoom error");
			return -1;
		}
	}
#endif
save_crop_obj:
	/*save crop_obj */
	zoom_crop.w = w;
	zoom_crop.h = h;
	zoom_crop.left = left;
	zoom_crop.top = top;

	zoom.data[UVC_CUR] = value;

	printf("ZOOM_DATA : value = %d \n",value);
	printf("zoom_crop.left is %d,zoom_crop.top is %d\n",zoom_crop.left,zoom_crop.top);
	ucamera_uvc_ct_attr_save(UVC_ZOOM_ABSOLUTE_CONTROL, value);
	return 0;
}

int sample_video_zoom_get(void)
{
	return zoom.data[UVC_CUR];
}

#endif

int sample_video_backlight_compens_set(int value)
{
	int ret;
	if (value < 0 || value > 10) {
		Ucamera_LOG("set BackLight Invalid leave:%d\n", value);
		return -1;
	}
	ret = IMP_ISP_Tuning_SetHiLightDepress(value);
	if (ret)
		Ucamera_LOG("ERROR: set BackLight Invalid leave:%d\n", value);
	backlight_compens.data[UVC_CUR] = value;
	ucamera_uvc_pu_attr_save(UVC_BACKLIGHT_COMPENSATION_CONTROL, value);

	return 0;
}

int sample_video_backlight_compens_get(void)
{
	uint32_t value = 0;

	if (!Ucam_Stream_On)
		return backlight_compens.data[UVC_CUR];
#ifdef T31
	int ret = -1;
	ret = IMP_ISP_Tuning_GetHiLightDepress(&value);
	if (ret)
		Ucamera_LOG("ERROR: get BackLight error:%d\n", ret);
#endif
	return value;
}

/*
* 0~100 -> 0~255
*/
int sample_video_brightness_set(int value)
{
	int ret;
	unsigned char bright = 0;
	unsigned char bright_set = 0;

	/*filter first set caused by enumeration*/
	uint64_t  timeuse = 0;
	struct timeval time_end;
	gettimeofday(&time_end, NULL);
	timeuse = (uint64_t)(1000000 * (time_end.tv_sec - time_start.tv_sec)) + time_end.tv_usec - time_start.tv_usec;
	if(timeuse < 1000000){
		return 0;
	}

	bright = value & 0xff;
#if 0
	bright_set = round(1.0 * bright * 255 / 100);
#else
	bright_set = round(1.0 * 1.5 * bright + 53);
#endif
	Ucamera_DEBUG("bright: %d, bright_set : %d \n",bright,  bright_set);
	ret = IMP_ISP_Tuning_SetBrightness(bright_set);
	if (ret)
		Ucamera_LOG("ERROR: set BrightNess failed :%d\n", ret);
	brightness.data[UVC_CUR] = bright;
	ucamera_uvc_pu_attr_save(UVC_BRIGHTNESS_CONTROL, bright);
	return 0;
}

int sample_video_brightness_get(void)
{
	int ret;
	unsigned char bright = 0;
	unsigned char bright_get = 0;

	Ucamera_DEBUG("#######brightness :%d\n",brightness.data[UVC_CUR]);
	if (!Ucam_Stream_On)
		return brightness.data[UVC_CUR];

	ret = IMP_ISP_Tuning_GetBrightness(&bright);
	if (ret) {
		Ucamera_LOG("get BrightNess failed:%d\n", ret);
		bright = 128;
	}
#if 0
	bright_get = round(1.0 * bright * 100 / 255);
#else
	bright_get = round(1.0 * (bright - 53) / 1.5);
#endif
	return bright_get;
}

/*
*0~100 -> 0~255
*/
int sample_video_contrast_set(int value)
{
#if 1
	int ret;
	unsigned char tmp = 0;
	unsigned char tmp_high = 0;
	unsigned char tmp_low = 0;
	IMPISPFixedContrastAttr fixedContrastAttr;

	tmp = value & 0xff;

	/* 230 ~ 255 */
	tmp_high = round(1.0 * (255 - tmp * 1.3 / 5.0));
	/* 25 ~ 0 */
	tmp_low = round(1.0 * (tmp * 1.3 / 5.0));

	Ucamera_DEBUG("contrast : %d, contrast_set :%d, %d\n", tmp, tmp_high, tmp_low);

	fixedContrastAttr.mode = 1;
	fixedContrastAttr.range_high = tmp_high;
	fixedContrastAttr.range_low = tmp_low;
	ret = IMP_ISP_SetFixedContraster(&fixedContrastAttr);
	if(ret < 0){
		IMP_LOG_ERR(TAG, "IMP_ISP_SetFixedContraster failed \n");
		return -1;
	}

	contrast.data[UVC_CUR] = tmp;
	ucamera_uvc_pu_attr_save(UVC_CONTRAST_CONTROL, tmp);
	return 0;
#else
	int ret;
	unsigned char tmp = 0;
	unsigned char tmp_set = 0;

	/*filter first set caused by enumeration*/
	uint64_t  timeuse = 0;
	struct timeval time_end;
	gettimeofday(&time_end, NULL);
	timeuse = (uint64_t)(1000000 * (time_end.tv_sec - time_start.tv_sec)) + time_end.tv_usec - time_start.tv_usec;
	if(timeuse < 1000000){
		return 0;
	}

	tmp = value & 0xff;
	/*215 ~ 255 */
	tmp_set = round(1.0 * 1.5 * tmp + 53);

	Ucamera_DEBUG("contrast : %d, contrast_set :%d\n", tmp, tmp_set);

	ret = IMP_ISP_Tuning_SetContrast(tmp_set);
	if(ret < 0){
		IMP_LOG_ERR(TAG, "IMP_ISP_Tuning_SetContrast failed \n");
		return -1;
	}

	contrast.data[UVC_CUR] = tmp;
	ucamera_uvc_pu_attr_save(UVC_CONTRAST_CONTROL, tmp);
	return 0;

#endif
}

int sample_video_contrast_get(void)
{
#if 0
	int ret;
	unsigned char tmp = 0;
	unsigned char tmp_get = 0;

	if (!Ucam_Stream_On)
	return contrast.data[UVC_CUR];

	ret = IMP_ISP_Tuning_GetContrast(&tmp);
	if (ret) {
		Ucamera_LOG("get Contrast failed:%d\n", ret);
		tmp = 128;
	}

	tmp_get = round(1.0 * (tmp - 53) / 1.5);
	return tmp_get;
#else

	Ucamera_DEBUG("#######contrast_get :%d\n",contrast.data[UVC_CUR]);
	return contrast.data[UVC_CUR];
#endif
}

/*
*0~100 -> 0~255
*/
int sample_video_saturation_set(int value)
{
	int ret;
	unsigned char tmp = 0;
	unsigned char tmp_set = 0;

	/*filter first set caused by enumeration*/
	uint64_t  timeuse = 0;
	struct timeval time_end;
	gettimeofday(&time_end, NULL);
	timeuse = (uint64_t)(1000000 * (time_end.tv_sec - time_start.tv_sec)) + time_end.tv_usec - time_start.tv_usec;
	if(timeuse < 1000000){
		return 0;
	}

	tmp = value & 0xff;
#if 0
	tmp_set = round(1.0 * tmp * 255 / 100);
#else
	tmp_set = round(1.0 * 1.5 * tmp + 53);
#endif
	Ucamera_DEBUG("saturation : %d, saturation_set :%d\n", tmp, tmp_set);

	ret = IMP_ISP_Tuning_SetSaturation(tmp_set);
	if (ret)
		Ucamera_LOG("set Saturation failed:%d\n", ret);
	saturation.data[UVC_CUR] = tmp;
	ucamera_uvc_pu_attr_save(UVC_SATURATION_CONTROL, tmp);
	return 0;
}

int sample_video_saturation_get(void)
{
	int ret;
	unsigned char tmp = 0;
	unsigned char tmp_get = 0;

	Ucamera_DEBUG("###### saturation_get :%d\n",saturation.data[UVC_CUR]);
	if (!Ucam_Stream_On)
		return saturation.data[UVC_CUR];

	ret = IMP_ISP_Tuning_GetSaturation(&tmp);
	if (ret) {
		Ucamera_LOG("get Saturation failed:%d\n", ret);
		tmp = 128;

	}

#if 0
	tmp_get = round(1.0 * tmp * 100 / 255);
#else
	tmp_get = round(1.0 * (tmp - 53) / 1.5);
#endif
	return tmp_get;
}

/*
*0~100 -> 0~255
*/
int sample_video_sharpness_set(int value)
{
	int ret;
	unsigned char tmp = 0;
	unsigned char tmp_set = 0;

	/*filter first set caused by enumeration*/
	uint64_t  timeuse = 0;
	struct timeval time_end;
	gettimeofday(&time_end, NULL);
	timeuse = (uint64_t)(1000000 * (time_end.tv_sec - time_start.tv_sec)) + time_end.tv_usec - time_start.tv_usec;
	if(timeuse < 1000000){
		return 0;
	}

	tmp = value & 0xff;

	if (tmp <= 50) {
		/* 53 ~ 128 */
		tmp_set = round(1.0 * 1.5 * tmp + 53);
	} else {
		/* 128 ~ 170 */
		tmp_set = round(1.0 * 0.84 * tmp + 86);
	}

	Ucamera_DEBUG("sharpness : %d, sharpness_set :%d\n", tmp, tmp_set);
	ret = IMP_ISP_Tuning_SetSharpness(tmp_set);
	if (ret)
		Ucamera_LOG("set Sharpness failed:%d\n", ret);
	sharpness.data[UVC_CUR] = tmp;
	ucamera_uvc_pu_attr_save(UVC_SHARPNESS_CONTROL, tmp);
	return 0;
}

int sample_video_sharpness_get(void)
{
	int ret;
	unsigned char tmp = 0;
	unsigned char tmp_get = 0;
	Ucamera_DEBUG("######sharpness_get :%d\n",sharpness.data[UVC_CUR]);

	if (!Ucam_Stream_On)
		return sharpness.data[UVC_CUR];

	ret = IMP_ISP_Tuning_GetSharpness(&tmp);
	if (ret) {
		Ucamera_LOG("get Sharpness failed:%d\n", ret);
		tmp = 128;
	}

	if (tmp <= 50) {
		tmp_get = round(1.0 * (tmp - 53) / 1.5);
	} else {
		tmp_get = round(1.0 * (tmp - 86) / 0.84);
	}

	return tmp_get;
}
#ifdef T31
int sample_video_hue_set(int value)
{
	int ret;
	unsigned char hue_value = value & 0xff;

	/*filter first set caused by enumeration*/
	uint64_t  timeuse = 0;
	struct timeval time_end;
	gettimeofday(&time_end, NULL);
	timeuse = (uint64_t)(1000000 * (time_end.tv_sec - time_start.tv_sec)) + time_end.tv_usec - time_start.tv_usec;
	if(timeuse < 1000000){
		return 0;
	}

	ret = IMP_ISP_Tuning_SetBcshHue(hue_value);
	if (ret)
		Ucamera_LOG("set Hue failed:%d\n", ret);
	hue.data[UVC_CUR] = hue_value;
	ucamera_uvc_pu_attr_save(UVC_HUE_CONTROL, hue_value);
	return 0;
}

int sample_video_hue_get(void)
{
	int ret;
	unsigned char hue_value = 0;

	if (!Ucam_Stream_On)
		return hue.data[UVC_CUR];
	ret = IMP_ISP_Tuning_GetBcshHue(&hue_value);
	if (ret) {
		Ucamera_LOG("set Hue failed:%d\n", ret);
		hue_value = 128;
	}
	return hue_value;
}
#endif
#if 0
int sample_video_gamma_set(int value)
{
	int ret;
	unsigned char tmp = 0;

	tmp = value & 0xff;
	ret = IMP_ISP_Tuning_SetContrast(tmp);
	if (ret)
		Ucamera_LOG("set Contrast failed:%d\n", ret);

	gamma_pu.data[UVC_CUR] = tmp;
	ucamera_uvc_pu_attr_save(UVC_GAMMA_CONTROL, tmp);
	return 0;
}

int sample_video_gamma_get(void)
{
	int ret;
	unsigned char cost = 0;

	if (!Ucam_Stream_On)
	return gamma_pu.data[UVC_CUR];

	ret = IMP_ISP_Tuning_GetContrast(&cost);
	if (ret) {
		Ucamera_LOG("get Contrast failed:%d\n", ret);
		cost = 128;
	}
	return cost;
}
#endif
int sample_video_gamma_set(int value)
{
	IMPISPGamma gamma;
	memset(&gamma, 0, sizeof(IMPISPGamma));

	memcpy(&gamma, &gamma_cur, sizeof(IMPISPGamma));
	gamma.gamma[64] = gamma_cur.gamma[64] + 3*(value - 400);
	IMP_ISP_Tuning_SetGamma(&gamma);

	gamma_pu.data[UVC_CUR] = value;
	ucamera_uvc_pu_attr_save(UVC_GAMMA_CONTROL, value);
	return 0;
}

int sample_video_gamma_get(void)
{
	return gamma_pu.data[UVC_CUR];
}

/*
* value高16位代表模式（0:自动/ 1:手动
* value 底16位为白平衡实际值
*/
int sample_video_whitebalance_set(int value)
{
	int ret;
	unsigned short gain, mode = 0;
	IMPISPWB wb = {0};

	mode = value >> 16;
	gain = value & 0xffff;

	if (mode == ISP_CORE_WB_MODE_AUTO)
		wb.mode = mode;
	else{
		wb.mode = ISP_CORE_WB_MODE_MANUAL;
		wb.bgain = gain;
		wb.rgain = 160000/gain;
	}
	ret = IMP_ISP_Tuning_SetWB(&wb);
	if (ret)
		Ucamera_LOG("set WhiteBalance failed:%d\n", ret);

	whitebalance.data[UVC_CUR] = wb.bgain;
	ucamera_uvc_pu_attr_save(UVC_WHITE_BALANCE_TEMPERATURE_CONTROL, wb.bgain);
	return 0;
}

int sample_video_hvfilp_set(int value)
{
	int ret = 0;
	unsigned long key_val = 0;

	key_val = sample_camera_off_pic_get_key_val();

	IMPISPHVFLIP testhv = value;
	/*camera_on need sethv */
	if (key_val) {
		ret = IMP_ISP_Tuning_SetHVFLIP(testhv);
		if (ret < 0){
			IMP_LOG_ERR(TAG, "failed to set HV Filp mode\n");
			CDC_LOG(UVC_SET_HVFLIP_ERROR," set fov hv filp error");
			return -1;
		}
	}

	Ucamera_DEBUG("Set HV filp mode:%d\n", value);
	hvflip.data[UVC_CUR] = value;
	ucamera_uvc_pu_attr_save(UVC_HVFLIP_CONTROL, value);

	/*usleep(100*1000);*/

	return 0;
}

int sample_video_hvfilp_get(void)
{
	int ret, tmp = 0;
	IMPISPHVFLIP hp = {0};

	ret = IMP_ISP_Tuning_GetHVFlip(&hp);
	if (ret)
		Ucamera_LOG("get hvfilp failed:%d\n", ret);
	tmp = hp;
	return tmp;
}

int sample_video_whitebalance_get(void)
{
	int ret, tmp = 0;
	IMPISPWB wb = {0};

	ret = IMP_ISP_Tuning_GetWB(&wb);
	if (ret)
		Ucamera_LOG("get WhiteBalance failed:%d\n", ret);
	if (wb.mode == ISP_CORE_WB_MODE_AUTO)
		wb.bgain = 417;
	tmp = (wb.mode << 16) | wb.bgain;
	return tmp;
}

int sample_video_powerlinefreq_set(int value)
{
	int ret, fps_num;
	uint32_t fps_num_bak,fps_den_bak;
	IMPISPAntiflickerAttr attr;

	attr = value;
	if (attr < IMPISP_ANTIFLICKER_DISABLE || attr >= IMPISP_ANTIFLICKER_BUTT) {
		Ucamera_LOG("[ERROR]Sample set PowerLine Freq Invalid level:%d\n", value);
		return 0;
	}
	Ucamera_LOG("Antiflicker is %d \n",attr);
	ret = IMP_ISP_Tuning_SetAntiFlickerAttr(attr);
	if (ret)
		Ucamera_LOG("set PowerLine Freq failed:%d\n", ret);

	ret = IMP_ISP_Tuning_GetSensorFPS(&fps_num_bak, &fps_den_bak);
	if (ret < 0) {
		Ucamera_LOG("failed to get sensor fps\n");
		return -1;
	}
	fps_num = fps_num_bak/fps_den_bak;
	if(fps_num <= 40){
		if (attr == IMPISP_ANTIFLICKER_50HZ)
			fps_num = SENSOR_FRAME_RATE_NUM_25;
		else if (attr == IMPISP_ANTIFLICKER_60HZ)
			fps_num = SENSOR_FRAME_RATE_NUM_30;
	} else {
		if (attr == IMPISP_ANTIFLICKER_50HZ)
			fps_num = 50;
		else if (attr == IMPISP_ANTIFLICKER_60HZ)
			fps_num = 60;
	}
	if(attr == IMPISP_ANTIFLICKER_DISABLE)
		fps_num = SENSOR_FRAME_RATE_NUM_30;

	ret = IMP_ISP_Tuning_SetSensorFPS(fps_num, 1);
	if (ret < 0) {
		Ucamera_LOG("failed to set sensor fps\n");
		return -1;
	}

	powerlinefreq.data[UVC_CUR] = attr;
	ucamera_uvc_pu_attr_save(UVC_POWER_LINE_FREQUENCY_CONTROL, attr);
	return 0;
}

int sample_video_powerlinefreq_get(void)
{
	int ret;
	IMPISPAntiflickerAttr attr;

	if (!Ucam_Stream_On)
		return powerlinefreq.data[UVC_CUR];

	ret = IMP_ISP_Tuning_GetAntiFlickerAttr(&attr);
	if (ret)
		Ucamera_LOG("get PowerLine Freq faild:%d\n", ret);
	return attr;
}
static int ucamera_cfg_sn_save()
{
	char key[64] = {0};
	char data[16] = {0};
	char line_str[128] = {0};
	char *des_str = NULL;
	FILE *fp = NULL;
	des_str = serial_label;
	if ((fp = fopen(UVC_CONFIG_BAK_PATH, "r+")) == NULL) {
		Ucamera_LOG("%s open config file failed!\n", __func__);
		return -1;
	}

	fseek(fp, 0L, SEEK_SET);

	while(!feof(fp)){
		if(fscanf(fp, "%[^\n]", line_str)<0)
			break;
		if(sscanf(line_str, "%[^:]:%[^\n]", key, data) != 2){
			Ucamera_LOG("warning: Invalid param:%s\n", line_str);
			fseek(fp, 1L, SEEK_CUR);
			continue;
		}
		char *ch = strchr(key, ' ');
		if (ch) *ch = 0;
		if(strcmp(key, "serial_lab") == 0){
			fseek(fp, -strlen(line_str), SEEK_CUR);
			fprintf(fp, "%s      :%s",key,des_str);
			break;
		}
		fseek(fp, 1L, SEEK_CUR);
	}
	fclose(fp);
	return 0;

}

int sample_video_eu_set(int cmd, void *data, int len)
{
	int ret = -1;
	char data_buf[len];
	char *buf;
	printf("###eu set cmd =%d, len =%d, ###\n",cmd ,len);
	memset(data_buf, 0, len);
	memcpy(data_buf, data, len);

	switch(cmd){
	case UVC_EU_CMD_USR13:
		{
		int value = 0;
		char cmd[64] = {0};
		if (data_buf[1] == 0x1 && data_buf[0] == 0x1) {
			ucamera_cfg_sn_save();
			printf("sn save finished\n");
			sprintf(cmd, "cp %s %s", UVC_CONFIG_BAK_PATH, UVC_CONFIG_PATH);
			/* printf("cmd = %s\n", cmd); */
			system(cmd);
			sleep(2);
			sprintf(cmd, "cp %s %s", UVC_ATTR_BAK_PATH, UVC_ATTR_PATH);
			system(cmd);
			/* printf("cmd = %s\n", cmd); */
			sleep(2);
			system("sync");
			system("reboot");
		} else {
			/* recovery and cdc log*/
			memcpy(&value, data, sizeof(int));
			if (value == 0x110) {
				ret = cdc_log_write();
			}

			ret =  set_norflash_flag(value);
			if(ret < 0)
				Ucamera_ERR("set cpm error\n");
		}

		}
		break;
	case UVC_EU_CMD_USR16:
		/* fov */ {
			Ucamera_DEBUG("data_buf[0] = 0x%d , data_buf[1] = 0x%d, data_buf[2] = 0x%d, data_buf[3] = 0x%d, data_buf[4] = 0x%d ###\n",data_buf[0], data_buf[1], data_buf[2], data_buf[3], data_buf[4]);
			int angle = 0;
			if (data_buf[4] == 0x0) {
				/*button mode */
				angle = data_buf[2];
				if (angle > 0 && angle <= SENSOR_DEFAULT_FOV) {
					Ucamera_DEBUG("fov is setting,the angle is %d",angle);
					sample_ucamera_set_fov(angle);
				} else {
					Ucamera_ERR("Unknown angle (%d)\n", angle);
				}

				angle = data_buf[2];
				zoom.data[UVC_CUR] = round(pow(tan((SENSOR_DEFAULT_FOV / 2) * PI / 180)/tan((angle / 2) * PI / 180 ),2)*100);
				/*ucamera_uvc_ct_attr_save(UVC_ZOOM_ABSOLUTE_CONTROL, zoom.data[UVC_CUR]);*/
				sample_video_zoom_set(zoom.data[UVC_CUR]);

				pantilt_mode = FOV_MODE;

			} else {
				/*pantilt : TODO*/
				pantilt_mode = PANTILT_MODE;
				/* reset mode */
				if (data_buf[7] == 1) {
					first_set_flag = 1;
					angle = data_buf[2];
					zoom.data[UVC_CUR] = round(pow(tan((SENSOR_DEFAULT_FOV / 2) * PI / 180)/tan((angle / 2) * PI / 180 ),2)*100);
					sample_video_zoom_set(zoom.data[UVC_CUR]);
					first_set_flag = 0;
				}
			}

			/* data_buf[6~5]: audio_angle */
			angle = (data_buf[6] << 8) | data_buf[5];
			sample_ucamera_set_audio_angle(angle);
			buf = (char*)&uvc_eu_attr.device_mode_set;
			memcpy(buf,data_buf,8);
			ucamera_uvc_ct_attr_save(EU_PANTILT_MODE_CONTROL,pantilt_mode);
		}
		break;
	case UVC_EU_CMD_USR17:
		{
		/* hvfilp */
		int value = 0;
		memcpy(&value, data, sizeof(int));
		ret = sample_video_hvfilp_set(value);
		if(ret < 0)
			Ucamera_ERR("set hvflip is faild\n");
		}
		break;
	case UVC_EU_CMD_USR18:
		/* powerlinefreq */
		{
		int value = 0;
		memcpy(&value, data, sizeof(int));
		if(value == 50)
			value = 1;
		else if(value == 60)
			value = 2;
		sample_video_powerlinefreq_set(value);
		}
		break;
	case UVC_EU_CMD_USR19:
		{
		int value = 0;
		memcpy(&value, data, sizeof(int));
		ucamera_hdr_change(value);
		}
		break;
	case UVC_EU_CMD_USR22:
		/*printf("############# \n");*/
		system("/system/init/dameon.sh");
		break;
	default:
		Ucamera_LOG("Unkown uvc eu cmd:%d\n", cmd);
	}
	return ret;
}

int sample_video_eu_get(int cmd, int req, void *data)
{
	uint8_t ret = 1, value = 0;
	uint8_t len = 1;
	uint8_t *data_cpy;
	char Kiva_version[LABEL_LEN] = KIVA_APP_VERSION;
	data_cpy = (uint8_t *)data;
	Ucamera_DEBUG("##########eu get cmd =%d, req=%d,%s\n", cmd, req, __func__);
	switch(cmd){
	case UVC_EU_CMD_USR12:
		switch(req){
#if 0
		case GET_LEN:
			len = strlen(Kiva_version);
			len = len + 1;
			memcpy(data_cpy, &len, sizeof(len));
			break;
#endif
		case GET_CUR:
			ret = strlen(Kiva_version);
			ret += 1;
			memcpy(data_cpy, Kiva_version, ret);
			break;
		}
		break;
	case UVC_EU_CMD_USR14:
		switch(req){
#if 0
		case GET_LEN:
			len = strlen(serial_label);
			len = len + 1;
			memcpy(data_cpy, &len, sizeof(len));
			break;
#endif
		case GET_CUR:
			ret = strlen(serial_label);
			ret += 1;
			memcpy(data_cpy, serial_label, ret);
			break;
		}
		break;
	case UVC_EU_CMD_USR16:
		switch(req){
#if 0
		case GET_LEN:
			len = 5;
			memcpy(data_cpy, &len, sizeof(len));
			break;
#endif
		case GET_CUR:
//			value = 16;
			/*ret = strlen(serial_label);*/
			memcpy(data_cpy, (void *)&uvc_eu_attr.device_mode_set, sizeof(uvc_eu_attr.device_mode_set));
			printf("data_copy is %d, %d ,%d, %d, %d, %d, %d, %d\n",data_cpy[0],data_cpy[1],data_cpy[2],data_cpy[3],data_cpy[4],data_cpy[5],data_cpy[6],data_cpy[7]);
			break;
		}
		break;
	case UVC_EU_CMD_USR17:
		switch(req){
#if 0
		case GET_LEN:
			len = 1;
			memcpy(data_cpy, &len, sizeof(len));
			break;
#endif
		case GET_CUR:
			value = hvflip.data[UVC_CUR];
			/*ret = strlen(serial_label);*/
			memcpy(data_cpy, (void *)&value, sizeof(value));
			break;
		}
		break;
	case UVC_EU_CMD_USR18:
		switch(req){
		case GET_LEN:
#if 0
			len = 1;
			memcpy(data_cpy, &len, sizeof(len));
			break;
#endif
		case GET_CUR:
			value = powerlinefreq.data[UVC_CUR];
			/*value = sample_video_powerlinefreq_get();*/
			if(value == 1)
				value = 50;
			else if(value == 2)
				value = 60;
			memcpy(data_cpy, (void *)&value, sizeof(value));
			break;
		}
		break;
	case UVC_EU_CMD_USR19:
		switch(req){
		case GET_CUR:
			value = uvc_eu_attr.hdr_mode;
			printf("the hdr_mode is %d\n",uvc_eu_attr.hdr_mode);
			memcpy(data_cpy, (void *)&value, sizeof(value));
			break;
		}
		break;
	default:
		memcpy(data, &len, sizeof(len));
		/*
		 *if(req == 0x81){
		 *        ret = strlen(serial_label);
		 *        memcpy(data_cpy, serial_label, strlen(serial_label));
		 *} else if(req == 0x85) {
		 *        len = strlen(serial_label);
		 *        memcpy(data_cpy, &len, sizeof(len));
		 *}
		 *if (req == 0x85)
		 *        [>data_cpy[0] = 1;<]
		 */
		break;
	}
	return ret;
}


int uvc_pu_attr_setcur(int type, int value)
{
	int ret = 0;
	struct Ucamera_Video_PU_Control *pu_attr = NULL;

	switch (type) {
	case UVC_BACKLIGHT_COMPENSATION_CONTROL:
		pu_attr = &backlight_compens;
		break;
	case UVC_BRIGHTNESS_CONTROL:
		pu_attr = &brightness;
		break;
	case UVC_CONTRAST_CONTROL:
		pu_attr = &contrast;
		break;
	case UVC_GAIN_CONTROL:
		break;
	case UVC_POWER_LINE_FREQUENCY_CONTROL:
		pu_attr = &powerlinefreq;
		break;
	case UVC_HUE_CONTROL:
		pu_attr = &hue;
		break;
	case UVC_HVFLIP_CONTROL:
		pu_attr = &hvflip;
		break;
	case UVC_SATURATION_CONTROL:
		pu_attr = &saturation;
		break;
	case UVC_SHARPNESS_CONTROL:
		pu_attr = &sharpness;
		break;
	case UVC_GAMMA_CONTROL:
		pu_attr = &gamma_pu;
		break;
	case UVC_WHITE_BALANCE_TEMPERATURE_CONTROL:
		pu_attr = &whitebalance;
		break;
	case UVC_WHITE_BALANCE_TEMPERATURE_AUTO_CONTROL:
		break;
	default:
		Ucamera_LOG("Unkown uvc pu type:%d\n", type);
		ret = -1;
		break;
	}
	if (pu_attr)
		pu_attr->data[UVC_CUR] = value;

	return ret;
}

int uvc_ct_attr_setcur(int type, int value)
{
	int ret = 0;
	struct Ucamera_Video_CT_Control *ct_attr = NULL;

	switch (type) {
	case UVC_ZOOM_ABSOLUTE_CONTROL:
		ct_attr = &zoom;
		if (ct_attr)
			ct_attr->data[UVC_CUR] = value;
		break;
	case UVC_PAN_ATTR_CONTROL:
		ct_attr = &pantilt;
		if (ct_attr) {
			uint32_t pan = value;
			uint64_t pantilt = ct_attr->ptdata[UVC_CUR];
			uint64_t hig = pantilt & 0xFFFFFFFF00000000 >> 32;

			pantilt = (hig << 32) | (0xFFFFFFFF & pan);

			/* printf("pantilt = %llx \n", pantilt); */
			ct_attr->ptdata[UVC_CUR] = pantilt;
		}
		break;
	case UVC_TILT_ATTR_CONTROL:
		ct_attr = &pantilt;
		if (ct_attr) {
			uint64_t tilt = value;
			uint64_t pantilt = ct_attr->ptdata[UVC_CUR];
			uint32_t low = pantilt & 0xFFFFFFFF;

			pantilt = ((0xFFFFFFFF & tilt) << 32 ) | low;

			/* printf("pantilt = %llx \n", pantilt); */
			ct_attr->ptdata[UVC_CUR] = pantilt;
		}
		break;
	case EU_FOV_ATTR_CONTROL:
		uvc_eu_attr.fov = value;
		uvc_eu_attr.device_mode_set = uvc_eu_attr.device_mode_set | ((uint64_t)value << 16);
		break;
	case EU_AUDIO_ANGLE_ATTR_CONTROL:
		uvc_eu_attr.audio_angle = value;
		uvc_eu_attr.device_mode_set = uvc_eu_attr.device_mode_set | ((uint64_t)value << 40);
		break;
	case EU_PANTILT_MODE_CONTROL:
		uvc_eu_attr.device_mode_set = uvc_eu_attr.device_mode_set | ((uint64_t)value<<32);
		pantilt_mode = value;
		break;
	case EU_HDR_MODE_CONTROL:
		uvc_eu_attr.hdr_mode = value;
		break;
	default:
		Ucamera_LOG("Unkown uvc ct type:%d\n", type);
		ret = -1;
		break;
	}

	return ret;
}
const char *uvc_ct_type_to_string(int type)
{
	return ct_string[type].s;
}

int uvc_ct_string_to_type(char *string)
{
	int i, index;
	index = sizeof(ct_string)/sizeof(struct uvc_ct_string);

	for (i = 0; i < index; i++) {
		if (strcmp(string, ct_string[i].s) == 0)
			return ct_string[i].id;
	}
	return 0;
}

const char *uvc_pu_type_to_string(int type)
{
	return pu_string[type].s;
}

int uvc_pu_string_to_type(char *string)
{
	int i, index;
	index = sizeof(pu_string)/sizeof(struct uvc_pu_string);

	for (i = 0; i < index; i++) {
		if (strcmp(string, pu_string[i].s) == 0)
			return pu_string[i].id;
	}
	return 0;
}

int ucamera_uvc_pu_attr_load(void)
{
	char key[64] = {0};
	char value[16] = {0};
	char line_str[128] = {0};
	int type;

	if ((access(UVC_ATTR_PATH, F_OK)) != 0) {
		/* backup files */
		char cmd[128] = {0};
		sprintf(cmd, "cp %s %s", UVC_ATTR_BAK_PATH, UVC_ATTR_PATH);
		system(cmd);
		sleep(1);
	}

	if ((uvc_attr_fd = fopen(UVC_ATTR_PATH, "r+")) == NULL) {
		Ucamera_LOG("%s open config file failed!\n", __func__);
		return -1;
	}

	while (!feof(uvc_attr_fd)) {
		if (fscanf(uvc_attr_fd, "%[^\n]", line_str) < 0)
			break;

		if (sscanf(line_str, "%[^:]:%[^\n]", key, value) != 2) {
			Ucamera_LOG("warning: skip config %s\n", line_str);
			fseek(uvc_attr_fd , 1L, SEEK_CUR);
			continue;
		}
		/* Ucamera_LOG("%s : %d\n", key, strToInt(value)); */
		if ((type = uvc_pu_string_to_type(key))) {
			uvc_pu_attr_setcur(type, strToInt(value));
		}
		if ((type = uvc_ct_string_to_type(key))) {
			uvc_ct_attr_setcur(type, htoi(value));
		}
		if(strcmp(key, "config_version") == 0){
			strncpy(config_version, value, LABEL_LEN);
		}
		fseek(uvc_attr_fd , 1L, SEEK_CUR);
	}

	return 0;
}

int ucamera_uvc_ct_attr_save(int type, int value)
{
	char key[64] = {0};
	char data[16] = {0};
	char line_str[128] = {0};
	const char *attr_string = NULL;


	if (uvc_attr_fd == NULL) {
		Ucamera_LOG("%s can not open uvc config file!\n", __func__);
		return -1;
	}
	attr_string = uvc_ct_type_to_string(type);
	if (attr_string == NULL)
		return -1;

	pthread_mutex_lock(&uvc_attr_mutex);
	fseek(uvc_attr_fd, 0L, SEEK_SET);

	while (!feof(uvc_attr_fd)) {
		if (fscanf(uvc_attr_fd, "%[^\n]", line_str) < 0)
			break;

		if (sscanf(line_str, "%[^:]:%[^\n]", key, data) != 2) {
			Ucamera_LOG("warning: Invalid param:%s\n", line_str);
			fseek(uvc_attr_fd , 1L, SEEK_CUR);
			continue;
		}
		if (strcmp(key, attr_string) == 0) {
			fseek(uvc_attr_fd, -strlen(line_str), SEEK_CUR);
			fprintf(uvc_attr_fd, "%s:%08x", key, value);
		}
		fseek(uvc_attr_fd , 1L, SEEK_CUR);
	}

	pthread_mutex_unlock(&uvc_attr_mutex);
	return 0;
}

int ucamera_uvc_pu_attr_save(int type, int value)
{
	char key[64] = {0};
	char data[16] = {0};
	char line_str[128] = {0};
	const char *attr_string = NULL;


	if (uvc_attr_fd == NULL) {
		Ucamera_LOG("%s can not open uvc config file!\n", __func__);
		return -1;
	}
	attr_string = uvc_pu_type_to_string(type);
	if (attr_string == NULL)
		return -1;

	pthread_mutex_lock(&uvc_attr_mutex);
	fseek(uvc_attr_fd, 0L, SEEK_SET);

	while (!feof(uvc_attr_fd)) {
		if (fscanf(uvc_attr_fd, "%[^\n]", line_str) < 0)
			break;

		if (sscanf(line_str, "%[^:]:%[^\n]", key, data) != 2) {
			Ucamera_LOG("warning: Invalid param:%s\n", line_str);
			fseek(uvc_attr_fd , 1L, SEEK_CUR);
			continue;
		}
		if (strcmp(key, attr_string) == 0) {
			fseek(uvc_attr_fd, -strlen(line_str), SEEK_CUR);
			fprintf(uvc_attr_fd, "%s:%04d", key, value);
		}
		fseek(uvc_attr_fd , 1L, SEEK_CUR);
	}

	pthread_mutex_unlock(&uvc_attr_mutex);
	return 0;
}

int imp_system_init(void)
{
	int ret;
	/* Step.1 System init */
	ret = sample_system_init();
	if (ret < 0) {
		IMP_LOG_ERR(TAG, "IMP_System_Init() failed\n");
		return -1;
	}
	return 0;
}

int imp_isp_tuning_init(void)
{
	int ret, fps_num;

	/* enable turning, to debug graphics */
	ret = IMP_ISP_EnableTuning();
	if(ret < 0){
		IMP_LOG_ERR(TAG, "IMP_ISP_EnableTuning failed\n");
		return -1;
	}

#if 0
	IMPISPAWBCluster awb_cluster;
	ret = IMP_ISP_Tuning_GetAwbClust(&awb_cluster);
	if (ret < 0) {
		IMP_LOG_ERR(TAG, "IMP_ISP_Tuning_GetAwbClust failed\n");
		return -1;
	}

	awb_cluster.ClusterEn = IMPISP_TUNING_OPS_MODE_ENABLE;
	ret = IMP_ISP_Tuning_SetAwbClust(&awb_cluster);
	if (ret < 0) {
		IMP_LOG_ERR(TAG, "IMP_ISP_Tuning_GetAwbClust failed\n");
		return -1;
	}
#endif

	/*0~100 -> 0 ~255 */
	unsigned char tmp_set = 0;
	unsigned char tmp = 0;

	sample_video_contrast_set(contrast.data[UVC_CUR]);

	sample_video_sharpness_set(sharpness.data[UVC_CUR]);

#if 0
	tmp_set = round(1.0 * saturation.data[UVC_CUR] * 255 / 100);
#else
	tmp = saturation.data[UVC_CUR];
	tmp_set = round(1.0 * 1.5 * tmp + 53);
#endif
	IMP_ISP_Tuning_SetSaturation(tmp_set);

#if 0
	tmp_set = round(1.0 * brightness.data[UVC_CUR] * 255 / 100);
#else
	tmp = brightness.data[UVC_CUR];
	tmp_set = round(1.0 * 1.5 * tmp + 53);
#endif
	IMP_ISP_Tuning_SetBrightness(tmp_set);

#ifdef T31
	/*IMP_ISP_Tuning_SetFrontCrop(&fcrop_obj);*/
    	IMP_ISP_Tuning_SetBcshHue(hue.data[UVC_CUR]);

	if(auto_exposure_mode.data[UVC_CUR] == 4)
		sample_video_exposure_time_set(exposure_time.data[UVC_CUR]);
	else
		IMP_ISP_Tuning_SetAeComp(128);
#endif

	IMPISPAntiflickerAttr attr = powerlinefreq.data[UVC_CUR];
	if (attr < IMPISP_ANTIFLICKER_DISABLE || attr >= IMPISP_ANTIFLICKER_BUTT) {
		Ucamera_LOG("%s unvaild Antiflicker param:%d\n", __func__, attr);
		attr = IMPISP_ANTIFLICKER_DISABLE;
	}
	IMP_ISP_Tuning_SetAntiFlickerAttr(attr);

	/*
	 *ret = IMP_ISP_Tuning_GetSensorFPS(&fps_num_bak, &fps_den_bak);
	 *if (ret < 0) {
	 *        Ucamera_LOG("failed to get sensor fps\n");
	 *        return -1;
	 *}
	 */

	fps_num = g_Fps_Num;
	if(fps_num <= 40){
		if (attr == IMPISP_ANTIFLICKER_50HZ)
			fps_num = SENSOR_FRAME_RATE_NUM_25;
		else if (attr == IMPISP_ANTIFLICKER_60HZ)
			fps_num = SENSOR_FRAME_RATE_NUM_30;
		/*
		 *else
		 *        fps_num = g_Fps_Num;
		 */
	} else {
		if (attr == IMPISP_ANTIFLICKER_50HZ)
			fps_num = 50;
		else if (attr == IMPISP_ANTIFLICKER_60HZ)
			fps_num = 60;
		/*
		 *else
		 *        fps_num = g_Fps_Num;
		 */
	}

	ret = IMP_ISP_Tuning_SetSensorFPS(fps_num, 1);
	if (ret < 0) {
		Ucamera_LOG("failed to set sensor fps\n");
		return -1;
	}
	Ucamera_LOG("set Antiflicker level:%d fps:%d\n", attr, fps_num);

	/* update ev status(used by dynamic_fps) */
	ev_last = 0;

	ret = IMP_ISP_Tuning_SetISPRunningMode(IMPISP_RUNNING_MODE_DAY);
	if (ret < 0){
		IMP_LOG_ERR(TAG, "failed to set running mode\n");
		return -1;
	}

#ifdef T31
	if ((hvflip.data[UVC_CUR] > IMPISP_FLIP_NORMAL_MODE) && (hvflip.data[UVC_CUR] < IMPISP_FLIP_MODE_BUTT)) {
		IMPISPHVFLIP testhv = hvflip.data[UVC_CUR];
		IMP_ISP_Tuning_SetHVFLIP(testhv);
		if (ret < 0){
			IMP_LOG_ERR(TAG, "failed to set HV Filp mode\n");
			return -1;
		}
		IMP_LOG_ERR(TAG, "Set HV filp mode:%d\n", hvflip.data[UVC_CUR]);

		usleep(100*1000);
	}
#else
	if (g_HV_Flip) {
		if (g_HV_Flip == 1)
			IMP_ISP_Tuning_SetISPHflip(IMPISP_TUNING_OPS_MODE_ENABLE);
		else if (g_HV_Flip == 2)
			IMP_ISP_Tuning_SetISPVflip(IMPISP_TUNING_OPS_MODE_ENABLE);
		else if (g_HV_Flip == 3){
			IMP_ISP_Tuning_SetISPHflip(IMPISP_TUNING_OPS_MODE_ENABLE);
			IMP_ISP_Tuning_SetISPVflip(IMPISP_TUNING_OPS_MODE_ENABLE);
		}else
			IMP_LOG_ERR(TAG, "Anvailed HV Filp mode :%d\n", g_HV_Flip);
	}
#endif
	return 0;

}

IMPRgnHandle *prHander;
int imp_sdk_init(int format, int width, int height)
{
	int i, ret;
	IMPFSChnAttr *imp_chn_attr_tmp;
	Ucamera_LOG("IMP SDK reinit!\n");
	zoom_cfg.width = width;
	zoom_cfg.height = height;
	imp_chn_attr_tmp = &chn[0].fs_chn_attr;

	/* set base info */
	imp_chn_attr_tmp->outFrmRateNum = g_Fps_Num;
	imp_chn_attr_tmp->picWidth = width;
	imp_chn_attr_tmp->picHeight = height;
	imp_chn_attr_tmp->nrVBs = 2;
	/* PS : resolution must be standard 16:9 or 4:3 */
#ifdef T31
	if(!uvc_eu_attr.hdr_mode){
		if(height == 360 ) {
			sensor_tuning(3, height);
			g_VideoWidth = 1280;
			g_VideoHeight = 720;
		}else{
			if(g_VideoHeight != g_VideoHeight_bk || g_VideoWidth != g_VideoWidth_bk){
				g_VideoHeight = g_VideoHeight_bk;
				g_VideoWidth = g_VideoWidth_bk;
				sensor_tuning(0,height);
			}
		}
	}

	/* T21/T31 : scaler, then crop */
	if (g_VideoWidth == g_VideoHeight * 4 / 3) {
		/* sensor setting - width : height = 4 : 3 */
		/*zoom need to open the scaler*/
		imp_chn_attr_tmp->scaler.enable    = 1;
		imp_chn_attr_tmp->scaler.outwidth  = width;
		imp_chn_attr_tmp->scaler.outheight = width * 3 / 4;

		/* set crop attr */
		if (width == (height * 16 / 9)) {
			imp_chn_attr_tmp->crop.enable = 1;
			imp_chn_attr_tmp->crop.left   = 0;
			imp_chn_attr_tmp->crop.top    = height / 6;
			imp_chn_attr_tmp->crop.width  = width;
			imp_chn_attr_tmp->crop.height = height;
		} else {
			imp_chn_attr_tmp->crop.enable = 0;
		}
	} else {
		/* sensor setting - width : height = 16 : 9 */
		/*zoom need to open the scaler*/
		imp_chn_attr_tmp->scaler.enable    = 1;
		imp_chn_attr_tmp->scaler.outwidth  = height * 16 / 9;
		imp_chn_attr_tmp->scaler.outheight = height;
	/*	if(height==36 && width== 640)
		{
			imp_chn_attr_tmp->scaler.outwidth  = 1280;
			imp_chn_attr_tmp->scaler.outheight = 720;
			printf("the width is 640p !!!!!!!!");
		}*/
		/* set crop attr */
		if (width == (height * 4 / 3)) {
			imp_chn_attr_tmp->crop.enable = 1;
			imp_chn_attr_tmp->crop.left   = width / 6;
			imp_chn_attr_tmp->crop.top    = 0;
			imp_chn_attr_tmp->crop.width  = width;
			imp_chn_attr_tmp->crop.height = height;
		} else {
			imp_chn_attr_tmp->crop.enable = 0;
		}
	}
#endif

	/* correct crop,scaler,top,left value to match soc platform */
#ifdef T31
	/* T31: width(scaler) 2 */
	if (imp_chn_attr_tmp->scaler.enable) {
		if (imp_chn_attr_tmp->scaler.outwidth % 2)
			imp_chn_attr_tmp->scaler.outwidth--;
	}
#elif defined T21
	/* T21: width 16, height 8, top 2, left 2 */
	if (imp_chn_attr_tmp->scaler.enable) {
		if (imp_chn_attr_tmp->scaler.outwidth % 16)
			imp_chn_attr_tmp->scaler.outwidth -= imp_chn_attr_tmp->scaler.outwidth % 16;
		if (imp_chn_attr_tmp->scaler.outheight % 8)
			imp_chn_attr_tmp->scaler.outheight -= imp_chn_attr_tmp->scaler.outheight % 8;
	}

	if (imp_chn_attr_tmp->crop.enable) {
		if (imp_chn_attr_tmp->crop.top % 2)
			imp_chn_attr_tmp->crop.top--;
		if (imp_chn_attr_tmp->crop.left % 2)
			imp_chn_attr_tmp->crop.left--;
		if (imp_chn_attr_tmp->crop.width % 16)
			imp_chn_attr_tmp->crop.width -= imp_chn_attr_tmp->crop.width % 16;
		if (imp_chn_attr_tmp->crop.height % 8)
			imp_chn_attr_tmp->crop.height -= imp_chn_attr_tmp->crop.height % 16;
	}
#elif defined T20
	/* no need correct */
#endif

	if (imp_chn_attr_tmp->crop.enable)
		Ucamera_LOG("IMP: Crop enable w:%d h:%d left:%d top:%d\n",
				imp_chn_attr_tmp->crop.width,
				imp_chn_attr_tmp->crop.height,
				imp_chn_attr_tmp->crop.left,
				imp_chn_attr_tmp->crop.top);

	if (imp_chn_attr_tmp->scaler.enable)
		Ucamera_LOG("IMP: Scaler enable w:%d h:%d\n",
				imp_chn_attr_tmp->scaler.outwidth,
				imp_chn_attr_tmp->scaler.outheight);

	if (g_Power_save) {
		ret = IMP_ISP_EnableSensor();
		if(ret < 0){
			IMP_LOG_ERR(TAG, "failed to EnableSensor\n");
			return -1;
		}
		imp_isp_tuning_init();

#if 1
		IMPISPFixedContrastAttr fixedContrastAttr;
		fixedContrastAttr.mode = 1;
		fixedContrastAttr.range_low = 13;
		fixedContrastAttr.range_high = 242;
		ret = IMP_ISP_SetFixedContraster(&fixedContrastAttr);
		if(ret < 0){
			IMP_LOG_ERR(TAG, "IMP_ISP_SetFixedContraster failed \n");
			return -1;
		}
#endif
	}


	/* Step.2 FrameSource init */
	ret = sample_framesource_init();
	if (ret < 0) {
		IMP_LOG_ERR(TAG, "FrameSource init failed\n");
		return -1;
	}

	if (format == V4L2_PIX_FMT_YUYV || format == V4L2_PIX_FMT_NV12) {
		IMPFSChnAttr fs_chn_attr[2];
		/* Step.3 Snap raw config */
		ret = IMP_FrameSource_GetChnAttr(0, &fs_chn_attr[0]);
		if (ret < 0) {
			IMP_LOG_ERR(TAG, "%s(%d):IMP_FrameSource_GetChnAttr failed\n", __func__, __LINE__);
			return -1;
		}

		fs_chn_attr[0].pixFmt = PIX_FMT_NV12;//PIX_FMT_YUYV422;
		ret = IMP_FrameSource_SetChnAttr(0, &fs_chn_attr[0]);
		if (ret < 0) {
			IMP_LOG_ERR(TAG, "%s(%d):IMP_FrameSource_SetChnAttr failed\n", __func__, __LINE__);
			return -1;
		}

		/* Step.3 config sensor reg to output colrbar raw data*/
		/* to do */

		/* Step.4 Stream On */
		if (chn[0].enable){
			ret = IMP_FrameSource_EnableChn(chn[0].index);
			if (ret < 0) {
				IMP_LOG_ERR(TAG, "IMP_FrameSource_EnableChn(%d) error: %d\n", ret, chn[0].index);
				return -1;
			}
		}

		/* Step.4 Snap raw */
		ret = IMP_FrameSource_SetFrameDepth(0, 1);
		if (ret < 0) {
			IMP_LOG_ERR(TAG, "%s(%d):IMP_FrameSource_SetFrameDepth failed\n", __func__, __LINE__);
			return -1;
		}
		IMP_ISP_Tuning_GetGamma(&gamma_cur);
		sample_video_gamma_set(gamma_pu.data[UVC_CUR]);
	Ucamera_DEBUG("first set fov");
        if (pantilt_mode) {
		first_set_flag = 1;
                sample_video_zoom_set(zoom.data[UVC_CUR]);
                usleep(100 * 1000);
                sample_video_pantilt_set((void *)(&pantilt.ptdata[UVC_CUR]));
		first_set_flag = 0;
        } else {
                sample_ucamera_set_fov(uvc_eu_attr.fov);
                //usleep(100 * 1000);
        }
		printf("!!!!!!!the fmt is %d \n", format);
		return 0;
	}

	for (i = 0; i < FS_CHN_NUM; i++) {
		if (chn[i].enable) {
			ret = IMP_Encoder_CreateGroup(chn[i].index);
			if (ret < 0) {
				IMP_LOG_ERR(TAG, "IMP_Encoder_CreateGroup(%d) error !\n", i);
				return -1;
			}
		}
	}

	/* Step.3 Encoder init */
	switch (format) {
	case V4L2_PIX_FMT_YUYV:
	case V4L2_PIX_FMT_NV12:
		break;
	case V4L2_PIX_FMT_MJPEG:
		ret = sample_jpeg_init();
		if (ret < 0) {
			IMP_LOG_ERR(TAG, "Encoder init failed\n");
			return -1;
		}
		break;
	case V4L2_PIX_FMT_H264:
		ret = sample_encoder_init();
		if (ret < 0) {
			IMP_LOG_ERR(TAG, "Encoder init failed\n");
			return -1;
		}
		/*增加OSD初始化代码*/
		if (IMP_OSD_CreateGroup(0) < 0) {
			IMP_LOG_ERR(TAG, "IMP_OSD_CreateGroup(%d) error !\n", 0);
			return -1;
		}
		prHander = sample_osd_init(0);
		if (prHander == NULL) {
			IMP_LOG_ERR(TAG, "OSD init failed\n");
			return -1;
		}
		break;
	}


	/* Step.4 Bind */
	for (i = 0; i < FS_CHN_NUM; i++) {
		if (chn[i].enable) {
			if (format == V4L2_PIX_FMT_H264) {
				IMPCell osdcell = {DEV_ID_OSD, 0, 0};
				ret = IMP_System_Bind(&chn[i].framesource_chn, &osdcell);
				if (ret < 0) {
					IMP_LOG_ERR(TAG, "Bind FrameSource channel%d and OSD failed\n",i);
					return -1;
				}

				ret = IMP_System_Bind(&osdcell, &chn[i].imp_encoder);
				if (ret < 0) {
					IMP_LOG_ERR(TAG, "Bind FrameSource channel%d and Encoder failed\n",i);
					return -1;
				}
			} else {
				ret = IMP_System_Bind(&chn[i].framesource_chn, &chn[i].imp_encoder);
				if (ret < 0) {
					IMP_LOG_ERR(TAG, "Bind FrameSource channel%d and Encoder failed\n",i);
					return -1;
				}
			}
		}
	}

	/* Step.5 Stream On */
	ret = sample_framesource_streamon();
	if (ret < 0) {
		IMP_LOG_ERR(TAG, "ImpStreamOn failed\n");
		return -1;
	}

	if (format == V4L2_PIX_FMT_H264) {
	#if 1
		ret = IMP_Encoder_StartRecvPic(chn[0].index);
		if (ret < 0) {
			IMP_LOG_ERR(TAG, "IMP_Encoder_StartRecvPic(%d) failed\n", chn[0].index);
			return -1;
		}
	#else
		sample_get_h264_start();
	#endif
	} else if (format == V4L2_PIX_FMT_MJPEG){
		ret = IMP_Encoder_StartRecvPic(3 + chn[0].index);
		if (ret < 0) {
			IMP_LOG_ERR(TAG, "IMP_Encoder_StartRecvPic:MJPEG(%d) failed\n", chn[0].index);
			return -1;
		}
	}
	IMP_ISP_Tuning_GetGamma(&gamma_cur);
	sample_video_gamma_set(gamma_pu.data[UVC_CUR]);

#if 1
	/*set pantit */

	Ucamera_DEBUG("first set fov");
        if (pantilt_mode) {
		first_set_flag = 1;
                sample_video_zoom_set(zoom.data[UVC_CUR]);
                usleep(100 * 1000);
                sample_video_pantilt_set((void *)(&pantilt.ptdata[UVC_CUR]));
		first_set_flag = 0;
        } else {
                sample_ucamera_set_fov(uvc_eu_attr.fov);
                //usleep(100 * 1000);
        }
#endif
	return 0;
}

int imp_sdk_deinit(int format)
{
	int i, ret;

	if (format == V4L2_PIX_FMT_YUYV || format == V4L2_PIX_FMT_NV12) {
		/* Step.5 Stream Off */
		if (chn[0].enable){
			ret = IMP_FrameSource_DisableChn(chn[0].index);
			if (ret < 0) {
				IMP_LOG_ERR(TAG, "IMP_FrameSource_DisableChn(%d) error: %d\n", ret, chn[0].index);
				return -1;
			}
		}

		/* Step.6 FrameSource exit */
		if (chn[0].enable) {
			/*Destroy channel i*/
			ret = IMP_FrameSource_DestroyChn(0);
			if (ret < 0) {
				IMP_LOG_ERR(TAG, "IMP_FrameSource_DestroyChn() error: %d\n", ret);
				return -1;
			}
		}
	} else {

		if (format == V4L2_PIX_FMT_H264) {
		#if 1
			ret = IMP_Encoder_StopRecvPic(chn[0].index);
			if (ret < 0) {
				IMP_LOG_ERR(TAG, "IMP_Encoder_StopRecvPic(%d) failed\n", chn[0].index);
				return -1;
			}
		#else
			sample_get_h264_stop();
		#endif
		} else if (format == V4L2_PIX_FMT_MJPEG) {
			ret = IMP_Encoder_StopRecvPic(3 + chn[0].index);
			if (ret < 0) {
				IMP_LOG_ERR(TAG, "IMP_Encoder_StopRecvPic:MJPEG(%d) failed\n", chn[0].index);
				return -1;
			}
		}

		/* Step.a Stream Off */
		ret = sample_framesource_streamoff();
		if (ret < 0) {
			IMP_LOG_ERR(TAG, "FrameSource StreamOff failed\n");
			return -1;
		}

		/* Step.b UnBind */
		IMPCell osdcell = {DEV_ID_OSD, 0, 0};
		for (i = 0; i < FS_CHN_NUM; i++) {
			if (chn[i].enable) {
				if (format == V4L2_PIX_FMT_H264) {
					ret = IMP_System_UnBind(&osdcell, &chn[i].imp_encoder);
					if (ret < 0) {
						IMP_LOG_ERR(TAG, "Bind OSD and Encoder failed\n");
						return -1;
					}

					ret = IMP_System_UnBind(&chn[i].framesource_chn, &osdcell);
					if (ret < 0) {
						IMP_LOG_ERR(TAG, "UnBind FrameSource and OSD failed\n");
						return -1;
					}
				} else {
					ret = IMP_System_UnBind(&chn[i].framesource_chn, &chn[i].imp_encoder);
					if (ret < 0) {
						IMP_LOG_ERR(TAG, "UnBind FrameSource channel%d and Encoder failed\n",i);
						return -1;
					}
				}
			}
		}

		/* Step.c Encoder exit */
		switch (format) {
		case V4L2_PIX_FMT_YUYV:
		case V4L2_PIX_FMT_NV12:
			break;
		case V4L2_PIX_FMT_MJPEG:
			ret = sample_jpeg_exit();
			break;
		case V4L2_PIX_FMT_H264:
			/*OSD exit*/
			ret = sample_osd_exit(prHander,0);
			if (ret < 0) {
				IMP_LOG_ERR(TAG, "OSD exit failed\n");
				return -1;
			}
			ret = sample_encoder_exit();
			break;
		}
		if (ret < 0) {
			IMP_LOG_ERR(TAG, "Encoder init failed\n");
			return -1;
		}

		for (i = 0; i < FS_CHN_NUM; i++) {
			if (chn[i].enable) {
				ret = IMP_Encoder_DestroyGroup(chn[i].index);
				if (ret < 0) {
					IMP_LOG_ERR(TAG, "IMP_Encoder_CreateGroup(%d) error !\n", i);
					return -1; }
			}
		}

		/* Step.d FrameSource exit */
		ret = sample_framesource_exit();
		if (ret < 0) {
			IMP_LOG_ERR(TAG, "FrameSource exit failed\n");
			return -1;
		}
	}

	if (g_Power_save) {
		ret = IMP_ISP_DisableTuning();
		if(ret < 0){
			IMP_LOG_ERR(TAG, "IMP_ISP_DisableTuning failed\n");
			return -1;
		}

		ret = IMP_ISP_DisableSensor();
		if(ret < 0){
			IMP_LOG_ERR(TAG, "failed to EnableSensor\n");
			return -1;
		}
	}
	focus.data[UVC_CUR] = 300;

	return 0;
}
static int ucamera_hdr_change(int cmd)
{
	if(pframe == NULL)
		return 0;
	uvc_eu_attr.hdr_mode = cmd;
	printf("!!!!!!the cmd is %d \n",cmd);
	Ucam_Stream_On = 0;
	imp_sdk_deinit(pframe->fcc);
	printf("stream off is finished!!!!\n");
	sensor_tuning(uvc_eu_attr.hdr_mode, pframe->height);

	printf("stream on is start!!!\n");
	imp_sdk_init(pframe->fcc,pframe->width,pframe->height);
	Ucam_Stream_On = 1;
	ucamera_uvc_ct_attr_save(EU_HDR_MODE_CONTROL, uvc_eu_attr.hdr_mode);
	return 0;
}
int imp_system_exit(void)
{
	int ret;
	/* Step.e System exit */
	ret = sample_system_exit();
	if (ret < 0) {
		IMP_LOG_ERR(TAG, "sample_system_exit() failed\n");
	}
	return ret;
}

int sample_get_ucamera_ev_cur()
{
	if (Ucam_Stream_On == 0)
		return 0;
	if (g_Dynamic_Fps)
		return ev_value;
	return 0;
}

int imp_backlight_Adapture(void)
{
	int value;
	int ret;
	value = sample_get_ucamera_ev_cur();
	if (!value)
		return 0;

	/* 重设背光补偿和强光抑制 */

	int blc = 0;
	ret = IMP_ISP_Tuning_GetBacklightComp(&blc);
	if (ret < 0){
		IMP_LOG_ERR(TAG, "failed to GetBacklightComp\n");
		return -1;
	}
	if (value > 10000) {
		blc = 0;
	} else {
		blc = 1;
	}

	ret = IMP_ISP_Tuning_SetBacklightComp(blc);
	if (ret < 0){
		IMP_LOG_ERR(TAG, "failed to SetBacklightComp\n");
		return -1;
	}

	ret = IMP_ISP_Tuning_GetBacklightComp(&blc);
	if (ret < 0){
		IMP_LOG_ERR(TAG, "failed to GetBacklightComp\n");
		return -1;
	}

	//printf("reset blc = %d \n", blc);

	sleep(1);

}

int imp_SensorFPS_Adapture(void)
{
	int ret, ev_cur = 0;
	int sensor_FPS = 0;
	IMPISPEVAttr evattr = {0};

	if (Ucam_Stream_On == 0)
		return 0;

	ret = IMP_ISP_Tuning_GetEVAttr(&evattr);
	if (ret < 0) {
		Ucamera_LOG("failed to get evattr\n");
		return -1;
	}
	//Ucamera_LOG("IMP get Ev:%d\n", evattr.ev);

	ev_value = evattr.ev;
	if (evattr.ev > 40000)
		ev_cur = 2;
#if 0
	else if (evattr.ev > 4000)
		ev_cur = 1;
#endif
	else
		ev_cur = 0;

	if (ev_cur == ev_last)
		return 0;

	ev_last = ev_cur;

	switch (ev_cur) {
	case 2:
		sensor_FPS = 15;
		break;
#if 0
	case 1:
		sensor_FPS = 15;
		break;
#endif
	case 0:
		sensor_FPS = g_Fps_Num;
		break;
	default:
		return 0;
	}

	ret = IMP_ISP_Tuning_SetSensorFPS(sensor_FPS, 1);
	if (ret < 0) {
		Ucamera_LOG("failed to set sensor fps\n");
		return -1;
	}

	return 0;
}


int ucamera_load_config(struct Ucamera_Cfg *ucfg )
{
	int nframes = 0;
	unsigned int i, width, height;
	int line_num = 0;
	char key[64] = {0};
	char value[64] = {0};
	char *line_str = NULL;
	struct Ucamera_Video_Cfg *vcfg;
	int intervals = 10000000/30;

	vcfg = &(ucfg->vcfg);
	if ((access(UVC_CONFIG_PATH, F_OK)) != 0) {
		/* backup files */
		char cmd[128] = {0};
		/*printf("sn save finished\n");*/
		ucamera_cfg_sn_save();
		sprintf(cmd, "cp %s %s", UVC_CONFIG_BAK_PATH, UVC_CONFIG_PATH);
		system(cmd);
		sleep(1);
	}

	if ((uvc_config_fd = fopen(UVC_CONFIG_PATH, "r+")) == NULL) {
		Ucamera_LOG("open config file failed %s!\n", __func__);
		return -1;
	}
	line_str = malloc(256*sizeof(char));
	if(line_str == NULL){
		Ucamera_LOG("malloc if line_str is failed\n");
		return -1;
	}

	/* printf("\n**********Config Param**********\n"); */
	while (!feof(uvc_config_fd)) {
		if (fscanf(uvc_config_fd, "%[^\n]", line_str) < 0)
			break;
		fseek(uvc_config_fd , 1, SEEK_CUR);
		line_num++;

		if (sscanf(line_str, "%[^:]:%[^\n]", key, value) != 2) {
			printf("warning: skip config %s\n", line_str);
			fseek(uvc_config_fd , 1, SEEK_CUR);
			continue;
		}

		char *ch = strchr(key, ' ');
		if (ch) *ch = 0;
		if (strcmp(key, "sensor_name") == 0) {
			/* printf("%s %s\n", key, value); */
			strncpy(g_Sensor_Name, value, sizeof(g_Sensor_Name));
#ifndef T20
		} else if(strcmp(key, "focus_trigger_value") == 0){
			focus_trigger_value = strToInt(value);
		} else if(strcmp(key,"motor_pixel") == 0) {
			motor_pixel = strToInt(value);
		} else if (strcmp(key, "af_on_off") == 0) {
			af_on_off = strToInt(value);
		} else if (strcmp(key, "motor_step") == 0) {
			motor_step = strToInt(value);
		} else if (strcmp(key, "motor_sleep") == 0) {
			motor_sleep_time = strToInt(value);
		} else if (strcmp(key, "motor_step_max") == 0) {
			motor_step_max = strToInt(value);
		} else if (strcmp(key, "motor_step_min") == 0) {
			motor_step_min = strToInt(value);
#endif
		} else if (strcmp(key, "i2c_addr") == 0) {
			/* printf("%s %s\n", key, value); */
			g_i2c_addr = htoi(value);
		} else if (strcmp(key, "fps_num") == 0) {
			/* printf("%s %s\n", key, value); */
			g_Fps_Num = strToInt(value);
		} else if (strcmp(key, "focus_max") == 0) {
			/* printf("%s %s\n", key, value); */
			focus.data[UVC_MAX] = strToInt(value);
		} else if (strcmp(key, "width") == 0) {
			/* printf("%s %s\n", key, value); */
			g_VideoWidth = strToInt(value);
			g_VideoWidth_bk = g_VideoWidth;
		} else if (strcmp(key, "height") == 0) {
			/* printf("%s %s\n", key, value); */
			g_VideoHeight = strToInt(value);
			g_VideoHeight_bk = g_VideoHeight;
		} else if (strcmp(key, "wdr_en") == 0) {
			/* printf("%s %s\n", key, value); */
			g_wdr = strToInt(value);
		} else if (strcmp(key, "bitrate") == 0) {
			/* printf("%s %s\n", key, value); */
			g_BitRate = strToInt(value);
		} else if (strcmp(key, "audio_en") == 0) {
			/* printf("%s %s\n", key, value); */
			g_Audio = strToInt(value);
			ucfg->audio_en = g_Audio;
		} else if (strcmp(key, "gop") == 0) {
			/* printf("%s %s\n", key, value); */
			g_gop = strToInt(value);
		} else if (strcmp(key, "qp_value") == 0) {
			/* printf("%s %s\n", key, value); */
			g_QP = strToInt(value);
		} else if (strcmp(key, "adb_en") == 0) {
			/* printf("%s %s\n", key, value); */
			g_adb = strToInt(value);
			ucfg->adb_en = g_adb;
		} else if (strcmp(key, "stillcap_en") == 0) {
			/* printf("%s %s\n", key, value); */
			ucfg->stillcap = strToInt(value);
		} else if (strcmp(key, "rndis_en") == 0) {
			/* printf("%s %s\n", key, value); */
			g_rndis = strToInt(value);
		} else if (strcmp(key, "dmic_en") == 0) {
			/* printf("%s %s\n", key, value); */
			g_dmic = strToInt(value);
		} else if (strcmp(key, "speak_en") == 0) {
			/* printf("%s %s\n", key, value); */
			g_Speak = strToInt(value);
		} else if (strcmp(key, "h264_en") == 0) {
			/* printf("%s %s\n", key, value); */
			ucfg->h264_en = strToInt(value);
		} else if (strcmp(key, "uvc_led") == 0) {
			/* printf("%s %s\n", key, value); */
			g_led = strToInt(value);
		} else if (strcmp(key, "led_level") == 0) {
			/* printf("%s %s\n", key, value); */
			led_level = strToInt(value);
		} else if (strcmp(key, "mic_volume") == 0) {
			/* printf("%s %s\n", key, value); */
			Mic_Volume = strToInt(value);
		} else if (strcmp(key, "speak_volume") == 0) {
			/* printf("%s %s\n", key, value); */
			Spk_Volume = strToInt(value);
			spk_volume_line = line_num;
		} else if (strcmp(key, "audio_ns") == 0) {
			/* printf("%s %s\n", key, value); */
			g_Audio_Ns = strToInt(value);
		} else if (strcmp(key, "hvflip") == 0) {
			/* printf("%s %s\n", key, value); */
			g_HV_Flip = strToInt(value);
		} else if (strcmp(key, "dynamic_fps") == 0) {
			/* printf("%s %s\n", key, value); */
			g_Dynamic_Fps = strToInt(value);
		} else if (strcmp(key, "device_bcd") == 0) {
			/* printf("%s %s\n", key, value); */
			DEVICE_BCD = strToInt_Hex(value);
		} else if (strcmp(key, "product_id") == 0) {
			/* printf("%s %s\n", key, value); */
			PRODUCT_ID = strToInt_Hex(value);
		} else if (strcmp(key, "vendor_id") == 0) {
			/* printf("%s %s\n", key, value); */
			VENDOR_ID = strToInt_Hex(value);
		} else if (strcmp(key, "serial_lab") == 0) {
			/* printf("%s %s\n", key, value); */
			strncpy(serial_label, value, LABEL_LEN);
		} else if (strcmp(key, "product_lab") == 0) {
			/* printf("%s %s\n", key, value); */
			strncpy(product_label, value, LABEL_LEN);
		} else if (strcmp(key, "manufact_lab") == 0) {
			/* printf("%s %s\n", key, value); */
			strncpy(manufact_label, value, LABEL_LEN);
		} else if (strcmp(key, "video_name") == 0) {
			 /*printf("%s %s\n", key, value);*/
			strncpy(video_device_name, value, LABEL_LEN);
		} else if (strcmp(key, "audio_name") == 0) {
			 /*printf("%s %s\n", key, value);*/
			strncpy(audio_device_name, value, LABEL_LEN);
		} else if (strcmp(key, "rcmode") == 0) {
			/* printf("%s %s\n", key, value); */
#ifdef T31
			if (strcmp(value, "vbr") == 0) {
				g_RcMode = IMP_ENC_RC_MODE_VBR;
			} else if (strcmp(value, "cbr") == 0) {
				g_RcMode = IMP_ENC_RC_MODE_CBR;
			} else if (strcmp(value, "fixqp") == 0) {
				g_RcMode = IMP_ENC_RC_MODE_FIXQP;
			} else if (strcmp(value, "cappedvbr") == 0) {
				g_RcMode = IMP_ENC_RC_MODE_CAPPED_VBR;
			} else {
				printf("Invalid RC method: %s\n", value);
			}
#else
			if (strcmp(value, "vbr") == 0) {
				g_RcMode = ENC_RC_MODE_VBR;
			} else if (strcmp(value, "cbr") == 0) {
				g_RcMode = ENC_RC_MODE_CBR;
			} else if (strcmp(value, "fixqp") == 0) {
				g_RcMode = ENC_RC_MODE_FIXQP;
			} else {
				printf("Invalid RC method: %s\n", value);
			}
#endif
		} else if (strcmp(key, "nframes") == 0) {
			/* printf("%s %s\n", key, value); */
			i = 0;
			struct Ucamera_YUYV_Param *yuyvl;
			struct Ucamera_JPEG_Param *jpegl;
			struct Ucamera_H264_Param *h264l;
			nframes = strToInt(value);
			vcfg->yuyvnum = nframes;
			vcfg->jpegnum = nframes;
			vcfg->h264num = nframes;
			yuyvl = malloc(vcfg->yuyvnum*sizeof(struct Ucamera_YUYV_Param));
			jpegl = malloc(vcfg->jpegnum*sizeof(struct Ucamera_JPEG_Param));
			h264l = malloc(vcfg->h264num*sizeof(struct Ucamera_H264_Param));
			intervals = 10000000/g_Fps_Num;
			int index = 0;

			while (i < nframes) {
				if (fscanf(uvc_config_fd, "%[^\n]", line_str) < 0)
					break;
				sscanf(line_str, "{%d, %d}", &width, &height);

				if (width > 0 && height > 0 && (width%16 == 0)) {

					/*skip yuyv 2k */
					if (width < 1280) {
						yuyvl[index].width = width;
						yuyvl[index].height = height;
						yuyvl[index].fps_num = intervals;
						index++;
					} else {
						vcfg->yuyvnum -= 1;
					}

					jpegl[i].width = width;
					jpegl[i].height = height;
					jpegl[i].fps_num = intervals;
					h264l[i].width = width;
					h264l[i].height = height;
					h264l[i].fps_num = intervals;
				} else {
					printf("error(%s %d)Invalid width or height(%d %d)\n", __func__, __LINE__, width, height);
				}
				i++;
				fseek(uvc_config_fd , 1, SEEK_CUR);
			}
			vcfg->yuyvlist = yuyvl;
			vcfg->jpeglist = jpegl;
			vcfg->h264list = h264l;
		} else {
			printf("Invalid config param: %s\n", key);
		}

	}
	/* printf("******************************\n"); */
	free(line_str);
	return 0;
}

int ucamera_config_version_save(char *config_version_bk)
{
	char key[64] = {0};
	char data[16] = {0};
	char line_str[128] = {0};
	unsigned int flag = 0;

	if (uvc_attr_fd == NULL) {
		Ucamera_LOG("%s can not open uvc config file!\n", __func__);
		return -1;
	}
	pthread_mutex_lock(&uvc_attr_mutex);
	fseek(uvc_attr_fd, 0L, SEEK_SET);

	while (!feof(uvc_attr_fd)) {
		if (fscanf(uvc_attr_fd, "%[^\n]", line_str) < 0)
			break;
		if (sscanf(line_str, "%[^:]:%[^\n]", key, data) != 2) {
			printf("warning: Invalid param:%s\n", line_str);
			fseek(uvc_attr_fd , 1L, SEEK_CUR);
			continue;
		}
		if (strcmp(key, "config_version") == 0) {
			fseek(uvc_attr_fd, -strlen(line_str), SEEK_CUR);
			fprintf(uvc_attr_fd, "%s:%s", key, config_version_bk);
			flag = 1;
		}
		fseek(uvc_attr_fd , 1L, SEEK_CUR);
	}
	if(!flag){
		fseek(uvc_attr_fd, 0L, SEEK_END);
		fprintf(uvc_attr_fd, "%s:%s\n","config_version", config_version_bk);
	}

	pthread_mutex_unlock(&uvc_attr_mutex);
	return 0;
}

static int config_version_checkout()
{
	char key[64] = {0};
	char value[64] = {0};
	char *line_str = NULL;
	FILE *uvc_attr_bk_fd;
	char config_version_bk[64] = {0};
	int audio_angle = 0;
	int ret ;

	line_str = malloc(256*sizeof(char));
	if(line_str == NULL){
		Ucamera_LOG("malloc if line_str is failed\n");
		return -1;
	}
	if((uvc_attr_bk_fd = fopen(UVC_ATTR_BAK_PATH, "r+")) == NULL){
		Ucamera_LOG("open config_bak file failed %s!\n", __func__);
		return -1;
	}

	while(!feof(uvc_attr_bk_fd)){
		ret = fscanf(uvc_attr_bk_fd, "%[^\n]", line_str);
		if (ret < 0)
			break;
		fseek(uvc_attr_bk_fd , 1, SEEK_CUR);

		if (sscanf(line_str, "%[^:]:%[^\n]", key, value) != 2) {
			Ucamera_LOG("warning: skip config %s\n", line_str);
			fseek(uvc_attr_bk_fd , 1, SEEK_CUR);
			continue;
		}
		if(strcmp(key, "audio_angle") == 0){
			audio_angle = htoi(value);
		} else if(strcmp(key, "config_version") == 0){
			strncpy(config_version_bk, value, 64);
		}
	}

	if(strcmp(config_version, config_version_bk) != 0)
	{
		uvc_eu_attr.audio_angle = audio_angle;
		sample_ucamera_set_audio_angle(uvc_eu_attr.audio_angle);
		uvc_eu_attr.device_mode_set &= ~(0xffll << 40);
		uvc_eu_attr.device_mode_set |= (uint64_t)audio_angle << 40;
		ucamera_config_version_save(config_version_bk);
	}
	fclose(uvc_attr_bk_fd);
	free(line_str);
	return 0;
}

static int kiva_generate_version()
{
	FILE *fp = NULL;
	int ret = -1;
	static char Kiva_version[LABEL_LEN] = KIVA_APP_VERSION;
	if ((fp = fopen("/tmp/version.txt", "w+")) == NULL) {
		Ucamera_LOG("open config file failed %s!\n", __func__);
		return -1;
	}

	ret = fwrite(Kiva_version, 1, strlen(Kiva_version), fp);
	if(ret != strlen(Kiva_version))
	{
		Ucamera_LOG("write  config file failed %s!\n", __func__);
		fclose(fp);
		return -1;
	}
	fclose(fp);
	return 0;
}

int speak_volume_write_config(int val)
{
	char value[24] = "speak_volume";
	char line_str[128] = {0};
	int line_num = 0;

	fseek(uvc_config_fd , 0L, SEEK_SET);
	while (!feof(uvc_config_fd)) {
		if (fscanf(uvc_config_fd, "%[^\n]", line_str) < 0){
			break;
		} else {
			fseek(uvc_config_fd , 1, SEEK_CUR);
			line_num++;
		}

		if (line_num == spk_volume_line - 1) {
			fprintf(uvc_config_fd, "%s    :%03d", value, val);
			} else
				continue;
		}
	return 0;

}

static int uvc_event_process(int event_id, void *data)
{
	unsigned int intervals = 0;
	unsigned long key_val = 0;

	switch (event_id) {
	case UCAMERA_EVENT_STREAMON: {

		CDC_LOG(UCAMERA_LOG_INFO, "UCAMERA_EVENT_STREAMON...");
		struct Ucamera_Video_Frame *frame = (struct Ucamera_Video_Frame *) data;
		pframe = frame;
		/* update the resution*/
		sample_update_resution(frame->width,frame->height);
		Ucam_Stream_On = 1;
		intervals = frame->intervals;
		g_Fps_Num = 10000000 / intervals;

		if (ucamera_uvc_pu_attr_load()) {
			Ucamera_LOG("[ERROR] load uvc PU attr failed.\n");
			CDC_LOG(UCAMERA_LOAD_ATTR_ERROR, "load uvc attr failed");
			return 0;
		}



		/* disable camera_on_off */
		sample_disable_camera_off_pic(frame->fcc);

		/* get key val and enable camera_on_off  */
		key_val = sample_camera_off_pic_get_key_val();
		if (!key_val) {
			sample_enable_camera_off_pic(frame->fcc);
			/* led control */
			if (g_led) {
				sample_ucamera_led_ctl(g_led, led_level);
			}
		} else {
			/* led control */
			if (g_led) {
				sample_ucamera_led_ctl(g_led, !led_level);
			}
		}

#ifndef T20
	int retry_cnt = 0;
imp_init_check:
		if (!imp_inited) {
			if (retry_cnt++ > 40) {
				Ucamera_LOG("[error]imp system init failed.\n");
				CDC_LOG(UVC_STREAMON_ERROR," imp system init failed");
				return 0;
			}
			Ucamera_LOG("imp sys not ready, wait and retry:%d", retry_cnt);
			usleep(50*1000);
			goto imp_init_check;
		}
#endif

#if SENSOR_RELOAD
			sensor_cutover(frame->fcc, frame->height, g_Fps_Num);
#endif
			/*set audio_angle */
			sample_ucamera_set_audio_angle(uvc_eu_attr.audio_angle);
			imp_sdk_init(frame->fcc, frame->width, frame->height);
			return 0;
	}
	case UCAMERA_EVENT_STREAMOFF: {
		CDC_LOG(UCAMERA_LOG_INFO, "UCAMERA_EVENT_STREAMOFF...");
		struct Ucamera_Video_Frame *frame = (struct Ucamera_Video_Frame *) data;
		pframe = frame;
		Ucam_Stream_On = 0;
		if (g_led)
			sample_ucamera_led_ctl(g_led, led_level);
		IMP_ISP_Tuning_SetGamma(&gamma_cur);

		return imp_sdk_deinit(frame->fcc);
	}

	default:
		Ucamera_LOG("%s(ERROR): unknown message ->[%d]!\n", TAG, event_id);
		return -1;
	};

	return 0;
}

FILE *pcm_fd = NULL;
FILE *ref_fd = NULL;
short ref_pcm[640] = {0};
int sample_audio_pcm_get(short *pcm)
{
	int len;
#if 0
	static int i = 0;
	if (pcm_fd == NULL)
		pcm_fd = fopen("/tmp/dmic.pcm", "wb");
	if (ref_fd == NULL)
		ref_fd = fopen("/tmp/ref.pcm", "wb");

#endif
	len = sample_audio_amic_pcm_get(ref_pcm);
#ifdef T31
	len = sample_audio_dmic_pcm_get(pcm);
#endif
#if 0
	if (i++ < 1200) {
		if (pcm_fd)
			fwrite(pcm, 2, len/2, pcm_fd);
		if (ref_fd)
			fwrite(ref_pcm, 2, len/2, ref_fd);
	}
#endif
	return len;
}


#ifndef T20
void signal_handler(int signum) {
	Ucamera_LOG("catch signal %d\n", signum);
	if (signum == SIGUSR1) {
		/*sem_post(&ucam_ready_sem);*/
	}
	else if(signum == SIGUSR2) {
		Ucamera_LOG("Ucamera Exit Now. \n");
		if(g_Audio == 1){
#ifdef T31
			if (g_dmic)
				sample_audio_dmic_exit();
			else
				sample_audio_amic_exit();
#endif
		}

		imp_system_exit();
		Ucamera_DeInit();
		/*Ucamera_Video_Stop();*/

		sample_camera_off_pic_deinit();
	} else if (signum == SIGKILL) {
		Ucamera_LOG("Ucamera Exit Now. \n");
		if(g_Audio == 1){
#ifdef T31
			if (g_dmic)
				sample_audio_dmic_exit();
			else
				sample_audio_amic_exit();
#endif
		}

		imp_system_exit();
		Ucamera_DeInit();
		/*Ucamera_Video_Stop();*/

		sample_camera_off_pic_deinit();
	}else if(signum == SIGSEGV){
		Ucamera_LOG("Ucamera Exit Now,the error is %s  \n","SIGSEGV");
		if(g_Audio == 1){
#ifdef T31
			if (g_dmic)
				sample_audio_dmic_exit();
			else
				sample_audio_amic_exit();
#endif
		}

		imp_system_exit();
		Ucamera_DeInit();
		/*Ucamera_Video_Stop();*/

		sample_camera_off_pic_deinit();
		exit(1);
	}


}
static void hdr_change_sensor()
{
	char sensor_name_1[16] = "sc500ai";
	char sensor_name_2[16] = "sc500ai_hdr";

	if(uvc_eu_attr.hdr_mode)
		strcpy(g_Sensor_Name,sensor_name_2);
	else
		strcpy(g_Sensor_Name,sensor_name_1);
	printf("!!!!!! the g_sensor is %s\n",g_Sensor_Name);
}
void *ucam_impsdk_init_entry(void *res)
{
	sem_wait(&ucam_ready_sem);
	hdr_change_sensor();
	sample_system_init();

	imp_inited = 1;
	if (g_Audio == 1) {
#ifdef T31
		if (g_dmic)
			sample_audio_dmic_init();
		else{
			sample_audio_amic_init();
			sample_ucamera_set_audio_angle(uvc_eu_attr.audio_angle);
		}
#endif
		Ucamera_Audio_Start();
	}

	if (g_Speak)
		sample_audio_play_start();
	return NULL;
}
#endif

int sample_enum_timestamp()
{
	gettimeofday(&time_start, NULL);
	return 0;
}

int sample_get_ucamera_streamonoff()
{
	return Ucam_Stream_On;
}

int sample_get_ucamera_af_onoff()
{
	return focus_auto.data[UVC_CUR];
}

int sample_get_ucamera_af_cur()
{
	return focus.data[UVC_CUR];
}

/* ---------------------------------------------------------------------------
 * main
 */

int main(int argc, char *argv[])
{
	struct Ucamera_Cfg ucfg;
	struct Ucamera_Video_CB_Func v_func;
	struct Ucamera_Audio_CB_Func a_func;
	struct Ucamera_Video_EU_Control euctl;
	memset(&ucfg, 0, sizeof(struct Ucamera_Cfg));
	memset(&v_func, 0, sizeof(struct Ucamera_Video_CB_Func));
	memset(&a_func, 0, sizeof(struct Ucamera_Audio_CB_Func));
	memset(&euctl, 0, sizeof(struct Ucamera_Video_EU_Control));

	/* Anticopy_verify */
#if 0
	if (AntiCopy_Verify()) {
		Ucamera_LOG("AntiCopy Verified failed!!!\n");
		return 0;
	}
#endif
	cdc_log_init();

	/* load uvc configs */
	if (ucamera_load_config(&ucfg)) {
		Ucamera_LOG("ucamera load config failed!\n");
		CDC_LOG(UCAMERA_LOAD_CONFIG_ERROR, "load config failed");
		return 0;
	}

	if (pthread_mutex_init(&uvc_attr_mutex, NULL) != 0){
		Ucamera_LOG("%s init mutex failed\n", __func__);
		return -1;
	}

	if (ucamera_uvc_pu_attr_load()) {
		Ucamera_LOG("[ERROR] load uvc PU attr failed.\n");
		CDC_LOG(UCAMERA_LOAD_ATTR_ERROR, "load uvc attr failed");
		return 0;
	}

	if(config_version_checkout()){
		Ucamera_LOG("config version checkout failed!\n");
		return 0;
	}
	/* load app version */
	if (kiva_generate_version())
		Ucamera_LOG("[Warning] kiva generate version is failed\n");

	/* uvc configs */
	if (g_Speak)
		ucfg.pcfg.Pid = PRODUCT_ID + 1;
	else
		ucfg.pcfg.Pid = PRODUCT_ID;
	ucfg.pcfg.Vid = VENDOR_ID;
	ucfg.pcfg.version = DEVICE_BCD;
	strcpy(ucfg.pcfg.manufacturer, manufact_label);
	strcpy(ucfg.pcfg.product, product_label);
	strcpy(ucfg.pcfg.serial, serial_label);
	strcpy(ucfg.pcfg.video_name, video_device_name);
	strcpy(ucfg.pcfg.audio_name, audio_device_name);

	ucfg.acfg.mic_volume = Dec_to_Hex(Mic_Volume);
	ucfg.acfg.spk_volume = Dec_to_Hex(Spk_Volume);
	if (g_Speak)
		ucfg.acfg.speak_enable = 1;
	else
		ucfg.acfg.speak_enable = 0;

	Ucamera_Config(&ucfg);
	Ucamera_Init(UVC_BUF_NUM, UVC_BUF_SIZE);

	v_func.get_YuvFrame = sample_get_yuv_snap_2;
	v_func.get_JpegFrame = sample_get_jpeg_snap_2;
	v_func.get_H264Frame = sample_get_h264_snap_2;
	Ucamera_Video_Regesit_CB(&v_func);

	//backlight_compens.set = sample_video_backlight_compens_set;
	//backlight_compens.get = sample_video_backlight_compens_get;
	brightness.set = sample_video_brightness_set;
	brightness.get = sample_video_brightness_get;
	contrast.set = sample_video_contrast_set;
	contrast.get = sample_video_contrast_get;
	saturation.set = sample_video_saturation_set;
	saturation.get = sample_video_saturation_get;
	sharpness.set = sample_video_sharpness_set;
	sharpness.get = sample_video_sharpness_get;
#ifdef T31
	hue.set = sample_video_hue_set;
	hue.get = sample_video_hue_get;
#endif
	//whitebalance.set = sample_video_whitebalance_set;
	//whitebalance.get = sample_video_whitebalance_get;
	powerlinefreq.set = sample_video_powerlinefreq_set;
	powerlinefreq.get = sample_video_powerlinefreq_get;
	gamma_pu.set = sample_video_gamma_set;
	gamma_pu.get = sample_video_gamma_get;

	focus.data[UVC_MIN] = motor_step_min;
	focus.data[UVC_MAX] = motor_step_max;
	focus.set = sample_video_focus_set;
	focus.get = sample_video_focus_get;
	focus_auto.set = sample_video_focus_auto_set;
	focus_auto.get = sample_video_focus_auto_get;
#ifndef T20
	exposure_time.set = sample_video_exposure_time_set;
	exposure_time.get = sample_video_exposure_time_get;
	auto_exposure_mode.set = sample_video_auto_exposure_mode_set;
	auto_exposure_mode.get = sample_video_auto_exposure_mode_get;
#endif
#ifdef T31
	zoom.set = sample_video_zoom_set;
	zoom.get = sample_video_zoom_get;

	pantilt.set1 = sample_video_pantilt_set;
	pantilt.get1 = sample_video_pantilt_get;
#endif
	euctl.set = sample_video_eu_set;
	euctl.get = sample_video_eu_get;

	/*filter the enum of set image*/
	sample_enum_timestamp();

	/* resgister uvc unit and terminal */
	Ucamera_Video_Regesit_Process_Unit_CB(Pu_ctrl);
	Ucamera_Video_Regesit_Camera_Terminal_CB(Ct_ctrl);
	Ucamera_Video_Regesit_Extension_Unit_CB(euctl);
	UCamera_Registe_Event_Process_CB(uvc_event_process);

	/* uvc video start */
	UCamera_Video_Start();

	if (g_Audio == 1) {
#ifdef T31
		if (g_dmic)
			a_func.get_AudioPcm = sample_audio_dmic_pcm_get;
		else
			a_func.get_AudioPcm = sample_audio_amic_pcm_get;
#else
			a_func.get_AudioPcm = sample_audio_amic_pcm_get;
#endif
		a_func.set_Mic_Volume = sample_set_mic_volume;
		a_func.set_Spk_Volume = sample_set_spk_volume;
		a_func.set_Mic_Mute = sample_set_mic_mute;
		a_func.set_Spk_Mute = sample_set_spk_mute;
		a_func.get_record_on = sample_get_record_on;
		a_func.get_record_off = sample_get_record_off;
		a_func.get_speak_on = sample_get_speak_on;
		a_func.get_speak_off = sample_get_speak_off;
		Ucamera_Audio_Regesit_CB(&a_func);
	}

	/* uvc leds */
	if (g_led)
		sample_ucamera_led_init(g_led, led_level);

	/* camera_on_off init */
	sample_camera_off_pic_init();

	sample_camera_off_pic_key_process();

#ifndef T20
	sem_init(&ucam_ready_sem, 0, 0);
	signal(SIGUSR1, signal_handler); /*set signal handler*/
	signal(SIGUSR2, signal_handler); /*set signal handler*/
	signal(SIGTERM, signal_handler); /*set signal handler*/
	signal(SIGKILL, signal_handler); /*set signal handler*/
	signal(SIGSEGV, signal_handler);

	/* sdk init */
	pthread_t ucam_impsdk_init_id;
	int ret;
	ret = pthread_create(&ucam_impsdk_init_id, NULL, ucam_impsdk_init_entry, NULL);
	if (ret != 0) {
		Ucamera_LOG("pthread_create failed \n");
		return -1;
	}
	/* af process */
	pthread_t tid_AF;
	pthread_attr_t attr;
	if(af_on_off == 1) {
		vcm_motor_param_t motor_param;
		memset(&motor_param, 0, sizeof(vcm_motor_param_t));
		motor_param.af_en = focus_auto.data[UVC_CUR];
		motor_param.motor_step_max = motor_step_max;
		motor_param.motor_step_min = motor_step_min;
		motor_param.motor_sleep_time = motor_sleep_time;
		motor_param.focus_trigger_value = focus_trigger_value;
		motor_param.motor_step = motor_step;
		motor_param.motor_pixel = motor_pixel;
		motor_param.af_cur = focus.data[UVC_CUR];

		sample_af_control_vcm_param_init(&motor_param);
		pthread_attr_init(&attr);
		pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_DETACHED);
		ret = pthread_create(&tid_AF, &attr, sample_af_control_process, NULL);
		if (ret != 0) {
			Ucamera_LOG("pthread_create af_control_process err \n");
			return -1;
		}
	}

#endif
	sem_post(&ucam_ready_sem);
	while (1) {
		if (g_Dynamic_Fps) {
			imp_SensorFPS_Adapture();
			if (uvc_eu_attr.hdr_mode) {
				imp_backlight_Adapture();
			}
		}

		 usleep(1000*1000*2);

	}
	return 0;
}
