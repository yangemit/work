...
int g_VideoWidth_bk = 1280;
int g_VideoHeight_bk = 720;
int g_i2c_addr = 0x1a;
int g_wdr = 0;
#ifdef T31
int g_RcMode = IMP_ENC_RC_MODE_CBR;
#else
int g_RcMode = ENC_RC_MODE_FIXQP;
#endif
int g_BitRate = 1000;
...
static int sample_reload_sensor_init(char *sensor_name_del, char *sensor_name_add)
{
	int ret;

	memset(&sensor_info_del, 0, sizeof(IMPSensorInfo));
	memcpy(sensor_info_del.name, sensor_name_del, strlen(sensor_name_del));
	sensor_info_del.cbus_type = TX_SENSOR_CONTROL_INTERFACE_I2C;
	memcpy(sensor_info_del.i2c.type, sensor_name_del, strlen(sensor_name_del));
	...
	memcpy(sensor_info_add.name,  sensor_name_add, strlen(sensor_name_add));
	sensor_info_add.cbus_type = TX_SENSOR_CONTROL_INTERFACE_I2C;
	memcpy(sensor_info_add.i2c.type, sensor_name_add, strlen(sensor_name_add));
...
static char sensor_last[16] = "sc500ai";
...
int sensor_cutover(int format, int height, int interval)
{
	...
	char sensor_name_2[16] = "sc500ai_2m";
...
int sensor_tuning(int cmd, int height)
{
	...
	char sensor_name_3[16] = "sc500ai_720";
...
int sample_system_init()
{
...
#endif
#endif
	strcpy(sensor_last, g_Sensor_Name);
	...
	IMP_OSD_SetPoolSize(1920 * 1080 * 4);
...
int sample_encoder_init()
{
    ...
    for (i = 0; i <  FS_CHN_NUM; i++) {
	    ...
	    IMPEncoderEncAttr *encAttr = &channel_attr.encAttr;
	    encAttr->eEncOptions = 0x40028;
...
int sample_get_h264_snap(char *img_buf)
{
	...
	if (ret < 0) {
		...
		CDC_LOG(IMP_POLLING_RESULT_ERROR, "IMP_Encoder_PollingStream (%s) timeout ", chnNum);
		return -1;
	}
	...
	if (ret < 0) {
		...
		CDC_LOG(IMP_ENCODER_GET_STREAM_ERROR, "IMP_Encoder_GetStream (%s) failed ", chnNum);
		return -1;
	}
	...
	for (i = 0; i < FS_CHN_NUM; i++) {
		if (chn[i].enable) {
			...
			if (ret < 0) {
				...
				CDC_LOG(IMP_POLLING_RESULT_ERROR, "IMP_Encoder_PollingStream (%s) timeout ", 3 + chn[i].index);
				continue;
			}
			...
			if (ret < 0) {
				...
				CDC_LOG(IMP_ENCODER_GET_STREAM_ERROR, "IMP_Encoder_GetStream (%s) failed ", 3 + chn[i].index);
				return -1;
			}
...
int sample_get_yuv_snap(char *img_buf)
{
	...
	ret = IMP_FrameSource_GetFrame(0, &frame_bak);
		...
		CDC_LOG(IMP_FRAMESOURCE_GET_STREAM_ERROR, "IMP_FrameSource_GetFrame (%s) failed ", 0);
		return -1;
    	}
...
void sample_audio_amic_init(void)
{
    ...
    int devID = 1;
    IMPAudioIOAttr attr;
    attr.samplerate = AUDIO_SAMPLE_RATE_48000;// AUDIO_SAMPLE_RATE_16000;
    attr.bitwidth = AUDIO_BIT_WIDTH_16;
    attr.soundmode = AUDIO_SOUND_MODE_MONO;
    attr.frmNum = 2;
    attr.numPerFrm = 960 * 4;
    attr.chnCnt = 1;
...
#endif
#if 0
static char *pcm_buf = NULL;
static FILE *pcm_file = NULL;
static int pcm_off = 0;
static int pcm_cnt = 0;
static int pcm_size = 192 * 1000 * 60;
#endif
#if 0
static struct timeval ts_ori, ts_file;
static unsigned long long before_ts, after_ts; 
#endif
int sample_audio_amic_pcm_get(short *pcm)
{
	...
	/*
	 *gettimeofday(&ts_ori, NULL);
	 *before_ts = (unsigned long long)ts_ori.tv_sec * 1000 + ts_ori.tv_usec / 1000;
	 */
	ret = IMP_AI_PollingFrame(devID, chnID, 1000);
	if (ret != 0 ) {
		IMP_LOG_ERR(TAG, "Audio Polling Frame Data error\n");
		CDC_LOG(IMP_AI_POLLING_RESULT_ERROR, "IMP_AI_PollingFrame error\n");
	}
	/*
	 *gettimeofday(&ts_file, NULL);
	 *after_ts = (unsigned long long)ts_file.tv_sec * 1000 + ts_file.tv_usec / 1000;
	 *printf("############## before: %llums, after: %llums, interval: %llums  ################\n", before_ts, after_ts, after_ts - before_ts);
	 */
	...
	if(ret != 0) {
		...
		CDC_LOG(IMP_AI_GET_FRAME_ERROR, "IMP_AI_GetFrame error\n");
		return 0;
	}
	...
	if (pcm_cnt < 1) {
		if (!pcm_buf) {
			pcm_buf = (char *)malloc(pcm_size);
			if (!pcm_buf) {
				printf("ERROR: pcm_buf malloc failed!\n");
				return -1;
			}
			memset(pcm_buf, 0, pcm_size);
		}
		memcpy(pcm_buf + pcm_off, frm.virAddr, frm.len);
		pcm_off += frm.len;

		/* save pcm file */
		if (pcm_off == pcm_size) {
			if (!pcm_file) {
				pcm_file = fopen("/tmp/pcm_test", "w");
				if (!pcm_file) {
					printf("ERROR: open pcm_file failed!\n");
					return -1;
				}
			}
			ssize_t w_len = fwrite(pcm_buf, 1, pcm_off, pcm_file); 
			if (w_len != pcm_off) {
				printf("ERROR: write pcm_file failed!\n");
				return -1;
			}
			pcm_off = 0;
			pcm_cnt++;
...
int sample_OSD_SetRgnAttr(int w, int h, IMPRgnHandle rHanderLogo, unsigned char * nv12_data)
{
	...
	rAttrLogo.rect.p1.x = w-1;     //p0 is start，and p1 well be epual p0+width(or heigth)-1
	rAttrLogo.rect.p1.y = h-1;
	rAttrLogo.fmt = PIX_FMT_NV12;
	rAttrLogo.data.picData.pData = nv12_data;
