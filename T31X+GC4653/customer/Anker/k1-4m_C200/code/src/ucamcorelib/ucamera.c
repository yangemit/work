...
static unsigned int g_nFormats = 2;
			   ...
			   uint8_t len, struct uvc_request_data *resp)
{
	...
	case UVC_INPUT_TERMINAL_ID:
		...
		case UVC_CT_PANTILT_ABSOLUTE_CONTROL:
			...
			case UVC_GET_MIN:
				resp->length = len;
				memcpy(&resp->data[0], &ctctl[cs-1].ptdata[UVC_MIN], resp->length);
			...
			case UVC_GET_MAX:
				resp->length = len;
				memcpy(&resp->data[0], &ctctl[cs-1].ptdata[UVC_MAX], resp->length);
			...
			case UVC_GET_CUR:
				{
					uint64_t ptvalue;
					if (ctctl[cs-1].get1 != NULL) {
						ptvalue = ctctl[cs-1].get1();
					}

					/*value = 2;*/
					resp->length = len;
					memcpy(&resp->data[0], &ptvalue, resp->length);
					/*
					 * For every successfully handled control
					 * request set the request error code to no
					 * error
			...
			case UVC_GET_DEF:
				{
					uint64_t ptvalue;
					ptvalue = ctctl[cs-1].ptdata[UVC_DEF];
					resp->length = len;
					memcpy(&resp->data[0], &ptvalue, resp->length);
					/*
					 * For every successfully handled control
			...
			case UVC_GET_INFO:
				...
				break;
			case UVC_GET_RES:
				value_2 = 0x0e1000000e10; /*36003600*/
				resp->length = len;
				memcpy(&resp->data[0], &value_2, resp->length);
				/*
				 * For every successfully handled control
				 * request, set the request error code to no
				 * error.
	...
	case UVC_EXTENSION_UNIT_ID:
			/*Ucamera_LOG("#########UVC: control request (req %s cs %02x) entity_id: %d########3\n", uvc_query_name(req), cs, entity_id);*/
			switch (req) {
				case UVC_SET_CUR:
					dev->puctl_cur = cs;
					dev->unit_cur = entity_id;
				...
				case UVC_GET_CUR:
					if (euctl.get != NULL)
					  	value = euctl.get(cs, req, (void *)resp->data);
					resp->length = len;
					/*printf("resp->length=%d, len=%d, data[0]=%d\n", resp->length, len, resp->data[0]);*/
				...
				case UVC_GET_LEN:
					...
					break;
				case UVC_GET_MAX:
					memset(resp->data, 255, 60);
					if(cs == UVC_EU_CMD_USR12 || cs == UVC_EU_CMD_USR14)
							resp->length = 0;
					else
							resp->length = len;
					dev->request_error_code.data[0] = 0x00;
...
static int uvc_update_ctl_data_parse(uint8_t cs, struct uvc_request_data *data)
{
	...
	if (euctl.set != NULL)
		euctl.set(cs, (void *)data->data, data->length);
	return 0;
}
				...
				struct uvc_request_data *data)
{
	...
	struct Ucamera_Video_PU_Control *puctl = dev->v_puctl;
	struct Ucamera_Video_CT_Control *ctctl = dev->v_ctctl;
	uint8_t	pt[data->length];
	memset(pt, 0, data->length);
	...
	case UVC_INPUT_TERMINAL_ID:
			...
			ctctl[cs-1].data[UVC_CUR] = value;
			break;
		case UVC_CT_PANTILT_ABSOLUTE_CONTROL:
			/* printf("data->length = %d\n", data->length); */
			memcpy(pt, data->data, data->length);
			if(ctctl[cs-1].set1 != NULL)
				ctctl[cs-1].set1(pt);
			ctctl[cs-1].ptdata[UVC_CUR] = pt;
			break;
		default:
			break;
		}
...
uvc_events_process_data(struct uvc_device *dev, struct uvc_request_data *data)
{
	...
	struct uvc_streaming_still_control *target1 = NULL;
...
uvc_events_process(struct uvc_device *dev)
{
	...
	case UVC_EVENT_STREAMON: {
		...
		if (dev->fcc == V4L2_PIX_FMT_YUYV || dev->fcc == V4L2_PIX_FMT_NV12) {
			dev->nbufs = 1;
		}
...
uvc_audio_events_process(struct uvc_device *dev)
{
	...
	case UAC_EVENT_RECORD_OFF:
		a_func->get_record_off();
		break;
	case UAC_EVENT_RECORD_ON:
		a_func->get_record_on();
		break;
	case UAC_EVENT_SPEAK_OFF:
		a_func->get_speak_off();
		break;
...
#if 0
#define PCM_FRM    (960 * 4 * 2)
static char *pcm_data = NULL;
static unsigned int pcm_off = 0;
static unsigned int pcm_new = 0;
static unsigned int pcm_cnt = 0;
static unsigned int pcm_len = PCM_FRM * 100; //4s
#endif
static void *usb_audio_recording_thread_entry(void *argv)
{
	...
	/*
	 *if (!pcm_data) {
	 *    pcm_data = (char *)malloc(pcm_len);
	 *    if (!pcm_data) {
	 *        printf("ERROR: pcm_data malloc failed!\n");
	 *        return;
	 *    }
	 *}
	 */
#if 0
#if 0
	FILE *fp_1k = fopen("/system/bin/1khz", "r");
	if (!fp_1k) {
		printf("#################### open 1khz failed ##################\n");
	}
	fseek(fp_1k, 0, SEEK_END);
	int tmp_len = ftell(fp_1k);
	char *tmp_buf = (char *)malloc(tmp_len);
	memset(tmp_buf, 0, tmp_len);
	fseek(fp_1k, 0, SEEK_SET);
	fread(tmp_buf, tmp_len, 1, fp_1k);
	/*int off = 0;*/
	int off = 192 * 1000;
#else
	FILE *fp_1k = fopen("/system/bin/1-40", "r");
	if (!fp_1k) {
		printf("#################### open 1khz failed ##################\n");
	}
	fseek(fp_1k, 0, SEEK_END);
...
#if 0
	int the_cnt = 0, act_cnt = 0;
	int switch_on = 0;
	struct timeval now_ts, rec_ts, ori_ts;
	unsigned long long now_tt, rec_tt, ori_tt;
#endif

	while(1) {
#if 1
		/*tmp_data = pcm_data + (pcm_new % pcm_len);*/
		/*len = a_func->get_AudioPcm(tmp_data);*/
		len = a_func->get_AudioPcm(pcm_buf);
		if (len <= 0) {
			usleep(10*1000); /*sleep 50 ms*/
			continue;
		}
		/*
		 *pcm_cnt++;
		 *pcm_new += len;
		 *if ((pcm_new - pcm_off) > pcm_len)
		 *    printf("WARNING: pcm_data is too much!\n");
		 */
#else
		if (!switch_on) {
			len = a_func->get_AudioPcm(pcm_buf);
			...
			switch_on = 1;
			gettimeofday(&ori_ts, NULL);
			ori_tt = (unsigned long long)ori_ts.tv_sec * 1000000 + ori_ts.tv_usec;
		}

		gettimeofday(&now_ts, NULL);
		now_tt = (unsigned long long)now_ts.tv_sec * 1000000 + now_ts.tv_usec;

		the_cnt = (ori_tt - now_tt) / (20 * 1000);
		if (the_cnt > act_cnt) {
			act_cnt++;
			len = 3840;
		} else {
...
#else
		ssize_t size = write(fd, tmp_buf + off, len);
		if (len != size) {
			if (0 != size)
				Ucamera_LOG("[ERROR]: gaudio write pcm(%d) failed,  actual(%d)\n", len, size);
		}
...
#if 0
static void *usb_audio_write_thread_entry(void *argv)
{
	int len = 0;
	int fd = 0;
	short pcm_buf[AUDIO_PCM_SIZE] = {0};
	...
	int off = 192 * 1000;
#else
	FILE *fp_1k = fopen("/system/bin/1-255-1", "r");
	if (!fp_1k) {
		printf("#################### open 1khz failed ##################\n");
	}
	fseek(fp_1k, 0, SEEK_END);
...
#if 1
		/*memset(pcm_buf, 0xaa, len);*/
		char *tmp_data = pcm_data + (pcm_off % pcm_len);
		ssize_t size = write(fd, tmp_data, len);
		if (len != size) {
			if (0 != size)
				Ucamera_LOG("[ERROR]: gaudio write pcm(%d) failed,  actual(%d)\n", len, size);
		}
...
#endif
#else
		memcpy(pcm_data + (pcm_new % pcm_len), pcm_buf, len);
		pcm_cnt++;
		pcm_new += len;
		if ((pcm_new - pcm_off) > pcm_len)
			printf("WARNING: pcm_data is too much!\n");
...
int Ucamera_Audio_Start(void)
{
	...
	/*pthread_t audio_thread_write_id;*/
	struct uvc_device *udev = g_udev;
...
#if 0
	ret = pthread_create(&audio_thread_write_id, NULL, usb_audio_write_thread_entry, udev);
	if(ret != 0) {
		Ucamera_LOG("[ERROR] %s: pthread_create Audio thread failed\n", __func__);
		return -1;
	}
...
#if 0
int set_norflash_flag(void)
{
	int ret = 0;
	int ucamera_fd = -1;
	int flags = UPGATE_SIGNATURE;
	/*hwvreg_write_paras hwvreg_paras; */
	struct erase_info erase;
	struct write_info write;


	ucamera_fd = open("/dev/mtd0", O_RDWR);
	if (ucamera_fd < 0) {
	...
	erase.start = NORFLASH_OFFSET;
	erase.length = 4;

	ret = ioctl(ucamera_fd, MEMERASE, &erase);
	if (ret) {
		printf("Write HWaddr:0x%08x value failed[%s].\n", erase.start,           strerror(errno));
		ret = -1;
		goto write_hwreg_err;
	}

/*
 *        hwvreg_paras.offset = NORFLASH_OFFSET;
 *        hwvreg_paras.value = 0;
 *        hwvreg_paras.len = sizeof(hwvreg_paras.value);
 *
 *        ret = ioctl(ucamera_fd, UCAMERA_IOCTL_READ_FLAG, &hwvreg_paras);
 *        if (ret) {
 *                printf("Read HWaddr:0x%08x value failed[%s].\n", hwvreg_paras.offset,            strerror(errno));
 *                ret = -1;
 *                goto read_hwreg_err;
 *        }
 *        printf("#######value=0x%x########\n", hwvreg_paras.value);
 *
 *        if (hwvreg_paras.value == UPGATE_SIGNATURE) {
 *                printf("Info(%s) : write cpm register for recovery success[%d]!\n", __func__,  __LINE__);
 *        } else {
 *                printf("Info(%s) : write cpm register for recovery failed[%d]!\n", __func__,   __LINE__);
 *                ret = -1;
 *                goto read_hwreg_err;
 *
 *        }
...
#endif
int set_norflash_flag(int value)
{
	int ret = 0;
	int ucamera_fd = -1;
	hwvreg_write_paras hwvreg_paras;
	...
	hwvreg_paras.offset = NORFLASH_OFFSET;
	if (value == 0x106)
		hwvreg_paras.value = UPGATE_SIGNATURE;
	else if(value == 0x110)
		hwvreg_paras.value = LOG_SIGNATURE;
	hwvreg_paras.len = sizeof(hwvreg_paras.value);

	ret = ioctl(ucamera_fd, UCAMERA_IOCTL_SET_FLAG, &hwvreg_paras);
	if (ret) {
		printf("Write HWaddr:0x%08x value failed[%s].\n", hwvreg_paras.offset,           strerror(errno));
		ret = -1;
		goto write_hwreg_err;
	...
	if (hwvreg_paras.value == UPGATE_SIGNATURE || hwvreg_paras.value == LOG_SIGNATURE) {
		printf("Info(%s) : write cpm register for recovery success[%d]!\n", __func__,  __LINE__);
	} else {
		printf("Info(%s) : write cpm register for recovery failed[%d]!\n", __func__,   __LINE__);
		ret = -1;
...
int write_norflash_data(void *data)
{
	int ret = 0;
	int ucamera_fd = -1;

	hwvreg_write_data *hwvreg_data = (hwvreg_write_data *)data;
	...
	printf("hwvreg_data->len = 0x%x, addr = 0x%p\n", hwvreg_data->len, hwvreg_data->data);

	ret = ioctl(ucamera_fd, UCAMERA_IOCTL_WRITE_NORFLASH, hwvreg_data);
	if (ret) {
		printf("Write HWaddr:0x%08x value failed[%s].\n", hwvreg_data->offset,           strerror(errno));
		ret = -1;
		goto write_hwreg_err;
	}

#if 0
	ret = ioctl(ucamera_fd, UCAMERA_IOCTL_READ_NORFLASH, hwvreg_data);
	if (ret) {
		printf("Write HWaddr:0x%08x value failed[%s].\n", hwvreg_data->offset,           strerror(errno));
		ret = -1;
		goto write_hwreg_err;
	...
	FILE *fp = fopen("/tmp/ttt.log", "w+");
	fwrite(hwvreg_data->data, hwvreg_data->len, 1, fp);
	fclose(fp);
#endif
...
int Ucamera_Init(int FrmNum, int FrmMaxSize)
{
	...
	for (i = 0; i < 20; i++) {
		...
		memset(ctctl[i].ptdata, 0, 8 * sizeof(uint64_t));
	}
	/* Init UVC events. */
...
int Ucamera_Config(struct Ucamera_Cfg *ucfg)
{
	...
	while (i < vcfg->yuyvnum) {
		...
		if (frame[cnt].height >= 1080)
			frame[cnt].intervals[0] = intervals*6;
		else if (frame[cnt].height >= 720)
	...
	while (i < vcfg->jpegnum) {
		...
		if(frame[cnt].height > 100){
			frame[cnt].intervals[0] = 10000000/30;
			/*frame[cnt].intervals[1] = 10000000/25;*/
		}else{
			frame[cnt].intervals[0] = 10000000/60;
			frame[cnt].intervals[1] = 10000000/30;
			/*
			 *frame[cnt].intervals[2] = 10000000/30;
			 *frame[cnt].intervals[3] = 10000000/25;
			 */
		}
...
int Ucamera_Video_Regesit_Camera_Terminal_CB(struct Ucamera_Video_CT_Control *ctctl[])
{
	...
	for (i = 0; (ctctl[i] != NULL) && (i < UVC_PRIVACY_CONTROL); i++) {
		ictl = ctctl[i];
		type = ictl->type;
		switch (type)
		{
			...
			case UVC_FOCUS_AUTO_CONTROL:
				...
				break;
			case UVC_PANTILT_ABSOLUTE_CONTROL:
				u_ictl[type-1].ptdata[UVC_MIN] = ictl->ptdata[UVC_MIN];
				u_ictl[type-1].ptdata[UVC_MAX] = ictl->ptdata[UVC_MAX];
				u_ictl[type-1].ptdata[UVC_DEF] = ictl->ptdata[UVC_DEF];
				u_ictl[type-1].ptdata[UVC_CUR] = ictl->ptdata[UVC_CUR];
				break;
		default:
		...
		u_ictl[type-1].get1 = ictl->get1;
		u_ictl[type-1].set1 = ictl->set1;
