...
#define AUDIO_DRIVER_VERSION "V21-20211219a"
...
static inline long dsp_ioctl_ai_stream(struct file *file, struct audio_input_stream *stream)
{
	...
	static unsigned int come_i = 0,come_out = 0;
	/*unsigned long lock_flags;*/
	...
	aecbuff = stream->aec;
	totalsize = stream->size;
	cnt = totalsize / dp->fragment_size;
...
//	printk("###################%d,%s\n",__LINE__,__func__);
	/* first copy */
	while(i < cnt){
		if(io_tracer+1 == dma_tracer || (dma_tracer==0 && io_tracer==dp->fragment_cnt-1))
			break;
		fragment = &(dp->fragments[io_tracer]);
		copy_to_user((stream->data + i * dp->fragment_size), fragment->vaddr, dp->fragment_size);
		dma_sync_single_for_device(NULL, fragment->paddr, dp->fragment_size, DMA_FROM_DEVICE);
		i++;
		io_tracer = (io_tracer + 1) % dp->fragment_cnt;
	}
	dp->io_tracer = io_tracer;
	/* second copy */
	if(i < cnt){
		dp->wait_flag = true;
		dp->wait_cnt = cnt - i - 1;
//		printk("###################%d,%s,dp->wait_cnt = %d\n",__LINE__,__func__,dp->wait_cnt);
		object->datanode.flags = SND_TASK_NODE_PREPARED;
		list_add_tail(&object->datanode.list, &dp->tasklist);
		mutex_unlock(&dp->mutex);
		do{
...
static void dsp_workqueue_handle(struct work_struct *work)
{
	...
	//		printk("###################%d,%s\n",__LINE__,__func__);
			loop_cnt0 = 0;
			dma_tracer = dpi->dma_tracer;
			io_tracer = dpi->io_tracer;
			while(io_tracer != dma_tracer){
				loop_cnt0++;
				io_tracer = (io_tracer + 1) % dpi->fragment_cnt;
			}
			if(loop_cnt0 >= dpi->wait_cnt){
	//			printk("###################%d,%s,loop_cnt0 = %d\n",__LINE__,__func__,loop_cnt0);
				dpi->wait_flag = false;
				node = list_first_entry(&dpi->tasklist, typeof(*node), list);
				node->flags = SND_TASK_NODE_FINISHED;
				list_del(&node->list);
...
static void dsp_dmic_workqueue_handle(struct work_struct *work)
{
	...
	if(dp_dmic && dp_dmic->is_trans && dp_dmic->pipe_state == SND_DSP_STATE_RUN){
		mutex_lock(&dp_dmic->mutex);
#if (defined(CONFIG_SOC_T30) || defined(CONFIG_SOC_T31))
		/* sync aec data */
...
int xb_snd_dsp_probe(struct snd_dev_data *ddata)
{
	...
	endpoints->expires = ns_to_ktime(1000*1000*10);	// the time section is 10ms.
