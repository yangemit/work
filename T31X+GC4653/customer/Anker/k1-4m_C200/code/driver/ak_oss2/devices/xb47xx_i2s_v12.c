...
static int dmic_channel = AUDIO_SOUND_MODE_MONO;
module_param(dmic_channel, int, S_IRUGO);
MODULE_PARM_DESC(dmic_channel, "control external codec channel ,1 --MODE_MONO, 2 -- MODE_STEREO ");
	...
	if (mode & CODEC_RMODE) {
		...
		if (cur_codec->codec_mode == CODEC_SLAVE) {
			...
			if (strcmp(cur_codec->name,"i2s_external_codec")){
				...
				printk("------ cur_codec->name=='i2s_external_codec'\n");
#endif
				return 0;
			}
...
//	__i2s_select_msbjustified(i2s_dev);
...
#if (defined(CONFIG_SOC_T21) || defined(CONFIG_SOC_T31))
		if (dmic_channel == AUDIO_SOUND_MODE_MONO) {
			__i2s_enable_monoctr_right(i2s_dev); /* 单声道 */
		} if (dmic_channel == AUDIO_SOUND_MODE_STEREO) {
			__i2s_disable_monoctr_right(i2s_dev);  /* 双声道 */
			__i2s_disable_monoctr(i2s_dev);
		} else {
			__i2s_enable_monoctr_right(i2s_dev); /* 单声道 */
		}

		/* printk("dmic_channel = %d \n", dmic_channel); */
#endif
	}
