...
static int dw9714_init_config(struct i2c_client *client)
{
	...
	ret = i2c_smbus_write_byte_data(client, 0xa1, 0x05);
	if (ret) {
		printk("i2c_smbus_write_byte_data err \n");
		goto err_i2c_write;
	}
	...
	byte1 = (unsigned char)(dac_value >> 4);
	byte2 = (unsigned char)( ((dac_value & 0x0f) << 4) | 0x04 );

	ret = i2c_smbus_write_byte_data(client, byte1, byte2);
	if (ret) {
		printk("i2c_smbus_write_byte_data err \n");
		goto err_i2c_write;
	}
...
static long dw9714_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd)
	{
		case MOTOR_MOVE:
			if (arg == 300){
				printk("*******motor reset to 300******** \n");
			}
			byte1 = (unsigned char)(arg >> 4);
			byte2 = (unsigned char)( ((arg & 0x0f) << 4) | 0x04 );
			ret = i2c_smbus_write_byte_data(dw9714->client, byte1, byte2);
			if (ret) {
				printk("i2c_smbus_write_byte_data err \n");
				return ret;
			}
