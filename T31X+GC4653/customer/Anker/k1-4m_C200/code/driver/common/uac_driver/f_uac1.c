...
static int dmic_channel = MIC_CH;
module_param(dmic_channel, int, S_IRUGO);
MODULE_PARM_DESC(dmic_channel, "dmic channel: 1 -- MODE_MONO, 2 --MODE_STEREO");
...
#ifdef CONFIG_GADGET_UAC1_CAP
	/* Set channel numbers */
	mic_input_terminal_desc.bNrChannels = dmic_channel;
	mic_as_type_i_desc.bNrChannels = dmic_channel;
...
static ssize_t gaudio_write(struct file *fp, const char __user *buf, size_t count, loff_t *ops)
{
	...
	//	printk("################### req_data_cnt : %d ###################\n", req_data_cnt);
		...
		if (req) {
			...
			req->length = len;
			/*dma_cache_sync(NULL, req->buf, len, DMA_TO_DEVICE);*/
...
int uac_set_descriptor(unsigned long arg)
{
	...
	/*set dmic channels and sample_rate */
	in_ep_actual_size = (in_sample_rate * dmic_channel * 2 * 40 / 1000);
	in_ep_req_count = (dmic_channel * 10);
	/* printk("dmic_channel = %d, in_sample_rate = %d \n", dmic_channel, in_sample_rate); */
