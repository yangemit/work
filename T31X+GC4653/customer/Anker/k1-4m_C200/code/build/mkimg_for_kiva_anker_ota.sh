...
PACKAGE_DIR=$EXT_DIR/package_generate/
...
################################
###   build package_generate ###
################################
build_package_generate ()
{
	echo "start build package_generate..."

	cd $PACKAGE_DIR
	make clean; make

	if [ $? != 0 ]; then
		echo "build package_generate... faild"
		exit 1
	fi

	cd -
	echo "build package_generate... ok...!"
}
...
pack_bin ()
{
	...
	if [ $ACM = y ]; then
		dd if=$FIRMWARE_BACKUP_DIR/u-boot-with-spl.bin of=$OUT_DIR/$FIRMWARE &&
			...
			dd if=$FIRMWARE_BACKUP_DIR/uImage0.ramdisk of=$OUT_DIR/$FIRMWARE  &&
				dd if=$FIRMWARE_BACKUP_DIR/appfs.jffs2 of=$OUT_DIR/$FIRMWARE bs=1k seek=4064 count=8192
	fi

	echo "build $SOLUTION img ok!"

	ls -lh $OUT_DIR/$FIRMWARE
	#cp $OUT_DIR/$FIRMWARE  $ANKER_DIR/$SOLUTION/code/build/Kiva_anker_update_$DATE.img

	echo "package_generate ota package!"
	cp $OUT_DIR/$FIRMWARE  $PACKAGE_DIR/firmware/firmware.bin
	cd $PACKAGE_DIR/
	./package_gen
	cp $PACKAGE_DIR/update_package.bin $ANKER_DIR/$SOLUTION/code/build/Kiva_anker_update_package_c200_$VERSION.img
	echo "package_generate ota package...ok!"
	cd -
}
