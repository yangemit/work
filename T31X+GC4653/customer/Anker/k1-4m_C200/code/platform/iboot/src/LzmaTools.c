...
static void *alloc_ptr = 0x80a00000;
void *lzalloc(void *x, unsigned items, unsigned size)
{
	void *p;
	...
	p = alloc_ptr;//malloc (size);
	alloc_ptr += size;

	return (p);
}

void lzfree(void *x, void *addr, unsigned nb)
{
	//free (addr);
}
				  ...
				  unsigned char *inStream,  SizeT  length)
{
	...
	/* debug("LZMA: Uncompresed size............ 0x%z0x.\n", outSizeFull); */
	/* debug("LZMA: Compresed size.............. 0x%z0x.\n", compressedSize); */

	g_Alloc.Alloc = lzalloc;
	g_Alloc.Free = lzfree;
