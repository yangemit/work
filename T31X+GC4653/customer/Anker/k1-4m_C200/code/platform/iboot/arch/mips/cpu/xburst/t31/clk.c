...
struct cgu cgu_clk_sel[CGU_CNT] = {
	...
	[AVPU] = {1, CPM_AVPUCDR, 30, MPLL, {APLL, MPLL, VPLL}, 29, 28, 27},
	[MACPHY] = {1, CPM_MACCDR, 30, MPLL, {APLL, MPLL, VPLL}, 29, 28, 27},
	[RSA] = {1, CPM_RSACDR, 30, MPLL, {APLL, MPLL, VPLL}, 29, 28, 27},
	...
	[I2S_SPK] = {1, CPM_I2SSPKCDR, 30, APLL, {APLL, MPLL, VPLL}, 29, 28, 27}, //需要再配置; i2s 使用APLL
	[I2S_MIC] = {1, CPM_I2SMICCDR, 30, APLL, {APLL, MPLL, VPLL}, 29, 28, 27}, //需要再配置; i2s 使用APLL
#ifdef CONFIG_BURNER
	[SSI] = {1, CPM_SSICDR, 30, CONFIG_CPU_SEL_PLL, {APLL, MPLL, VPLL}, 28, 27, 26}, /* TODO */
#else
	[SSI] = {1, CPM_SSICDR, 30, MPLL, {APLL, MPLL, VPLL}, 28, 27, 26},
#endif
	[CIM] = {1, CPM_CIMCDR, 30, VPLL, {APLL, MPLL, VPLL}, 29, 28, 27},
	[ISP] = {1, CPM_ISPCDR, 30, VPLL, {APLL, MPLL, VPLL}, 29, 28, 27},
};
...
static unsigned int pll_get_rate(int pll)
{
    ...
    /* Decimal counting is no longer supported(不在支持小数计算) */
	switch (pll) {
	case APLL:
