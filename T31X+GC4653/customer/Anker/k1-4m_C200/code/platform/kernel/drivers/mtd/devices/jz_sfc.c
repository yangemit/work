...
/*#ifdef CONFIG_USB_UPDATE*/
static int recovery_norflash_erase(struct erase_info *instr)
{
	int ret;
	uint32_t addr, end;

	mutex_lock(&Recoveryflash->lock);

	addr = (uint32_t)instr->addr;
	end = addr + (uint32_t)instr->len;

	ret = jz_spi_norflash_wait_till_ready(Recoveryflash);
	if (ret) {
		printk("spi wait timeout !\n");
		mutex_unlock(&Recoveryflash->lock);
		return ret;
	}

	while (addr < end) {
		ret = jz_spi_norflash_erase_sector(Recoveryflash, addr);
		if (ret) {
			printk("erase error !\n");
			mutex_unlock(&Recoveryflash->lock);
		...
		addr += Recoveryflash->mtd.erasesize;
	}
...
EXPORT_SYMBOL(direct_erase_norflash);
...
static int jz_spi_norflash_erase(struct mtd_info *mtd, struct erase_info *instr)
{
	...
	if (addr) {
		printk("%s eraseaddr no align,addr=0x%x, erasesize=0x%x\n", __func__, instr->addr, mtd->erasesize);
...
/*#ifdef CONFIG_USB_UPDATE*/
int recovery_norflash_read(loff_t from, size_t len, unsigned char *buf)
{
	int ret,j;
	unsigned char command_stage1[5];
	struct sfc_transfer transfer[1];
	...
	swap_buf = Recoveryflash->swap_buf;

	mutex_lock(&Recoveryflash->lock);

	jz_spi_norflash_set_address_mode(Recoveryflash,1);

#ifdef CONFIG_SPI_QUAD
	if(quad_mode == 0)
		jz_spi_norflash_set_quad_mode(Recoveryflash);
#endif


#ifdef CONFIG_SPI_QUAD
	command_stage1[0] = Recoveryflash->board_info->quad_mode->cmd_read;//SPINOR_OP_READ_1_1_4;
#elif CONFIG_SPI_STARDARD_FAST
	command_stage1[0] = SPINOR_OP_READ_FAST;//SPINOR_OP_READ_FAST;
#else
	...
	if(len <= SWAP_BUF_SIZE){
		...
		*/
		for(j = 1; j <= Recoveryflash->addr_len; j++){
			command_stage1[j] = from >> (Recoveryflash->addr_len - j) * 8;
		}
		...
		ret = jz_sfc_pio_txrx(Recoveryflash, transfer);
		if(ret != transfer[0].len)
			dev_err(Recoveryflash->dev,"the transfer length is error,%d,%s\n",__LINE__,__func__);
		...
		while(!(len  == tmp_len)){
			...
			*/
			for(j = 1; j <= Recoveryflash->addr_len; j++){
				command_stage1[j] = (from + tmp_len) >> (Recoveryflash->addr_len - j) * 8;
			}
	...
	jz_spi_norflash_set_address_mode(Recoveryflash,0);
...
EXPORT_SYMBOL(direct_read_norflash);
...
/*#ifdef CONFIG_USB_UPDATE*/
int recovery_norflash_write(loff_t to, size_t len, const unsigned char *buf)
{
	unsigned int i, j, ret, err;
	int ret_addr, actual_len, write_len;
	unsigned char command_stage1[5];
	...
	ret_addr = (to & (Recoveryflash->mtd.writesize - 1));


	if(Recoveryflash->mtd.writesize - ret_addr > len)
		actual_len = len;
	else
		actual_len = Recoveryflash->mtd.writesize - ret_addr;
...
#endif
	/* less than mtd->writesize */
		ret = jz_spi_norflash_write_enable(Recoveryflash);
		if (ret) {
			printk("write enable error !\n");
			mutex_unlock(&Recoveryflash->lock);
			return ret;
		...
		*/
		for(j = 1; j <= Recoveryflash->addr_len; j++){
			command_stage1[j] = to >> (Recoveryflash->addr_len - j) * 8;
		}
		...
		err = jz_spi_norflash_wait_till_ready(Recoveryflash);
		if (err) {
			printk("wait timeout !\n");
			mutex_unlock(&Recoveryflash->lock);
			return err;
	...
	for (i = actual_len; i < len; i += Recoveryflash->mtd.writesize) {
		ret = jz_spi_norflash_write_enable(Recoveryflash);
		if (ret) {
			printk("write enable error !\n");
			mutex_unlock(&Recoveryflash->lock);
		...
		*/
		for(j = 1; j <= Recoveryflash->addr_len; j++){
			command_stage1[j] = (to + i) >> (Recoveryflash->addr_len - j) * 8;
		}

		if(len - i < Recoveryflash->mtd.writesize)
			write_len = len - i;
		else
			write_len = Recoveryflash->mtd.writesize;
...
EXPORT_SYMBOL(direct_write_norflash);
