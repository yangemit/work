...
#ifdef CONFIG_PRODUCT_IGOR
static struct uart_jz47xx_port *up_global = NULL;
...
static int  serial_transmit_private(char* buf, int count)
{
	unsigned int ier;
	/* save the eir status */
	int ier_status = readl(0xB0030000 + (UART_IER << 2));
	/* printk("read ier_status:%d!!!\n", ier_status); */
	int i = 0;

	unsigned int status, tmout = 10000;

	ier = ier_status & (~UART_IER_THRI);
	/* printk("UART_IER_THRI ier:%d!!!\n", ier); */

	writel(ier, 0xB0030000 + (UART_IER << 2));

	while (i < count) {
		/* wait the last transmit done */
		/* Wait up to 10ms for the character(s) to be sent. */
		do {
			status = readl(0xB0030000 + (UART_LSR << 2));
			/*	/\* if (status & UART_LSR_BI) *\/ */
			/*		/\*TODO: up->lsr_break_flag = UART_LSR_BI; *\/ */
		...
		/*transfer one byte  */
		writel(*(char *)(buf + i), 0xB0030000 + (UART_TX << 2));
		i ++;
	}
	...
	writel(ier_status, 0xB0030000 + (UART_IER << 2));
	return i;
}



static ssize_t proc_state_show(struct file *file, char __user *buf,
...
static ssize_t proc_state_store(struct file *file, const char __user *buf,
				size_t size, loff_t *off)
{
	int ret;
	memset(buf_img, 0, IMG_SIZE);
	if (copy_from_user(buf_img, buf, size)){
		printk("copy from user size %d failed!!!\n", size);
	} else {
		printk("copy from user size %d success!!!\n", size);
	}

	/* spin_lock(&up_global->port.lock); */
	unsigned long flags;
	spin_lock_irqsave(&up_global->port.lock, flags);
	/* serial_out(up_global, UART_IER, up_global->ier); */
	ret = serial_transmit_private(buf_img, size);
	/* spin_unlock(&up_global->port.lock); */
	spin_unlock_irqrestore(&up_global->port.lock, flags);
...
static const struct file_operations proc_state_fops = {
	.owner		= THIS_MODULE,
	.read		= proc_state_show,
	.write		= proc_state_store,
};
#endif
...
#ifdef CONFIG_PRODUCT_IGOR
	static struct proc_dir_entry *proc_uart0_dir = NULL;
#endif
...
#ifdef CONFIG_PRODUCT_IGOR
	/* create proc */
	proc_uart0_dir = proc_mkdir("mytest", NULL);
	if (proc_uart0_dir) {
		proc_create("test", 0, proc_uart0_dir, &proc_state_fops);
	}

	buf_img = kmalloc(IMG_SIZE, GFP_KERNEL);
#endif
	return ret;
}
