...
static int _ipu_set_bg_route(struct jz_ipu *ipu, struct ipu_param *ipu_param)
{
    ...
    switch (infmt) {
        case IN_FMT_RGB_888: /* BGRA need srcw<<2,register Y_STRIDE & OUT_STRIDE don't neet srcw<<2 */
            srcw = srcw<<2;
            break;
        default:
            srcw = srcw;
	...
	*/
    ...
    } else if (ip->out_fmt == HAL_PIXEL_FORMAT_BGRA_8888 || ip->out_fmt == HAL_PIXEL_FORMAT_RGBA_8888 || ip->out_fmt == HAL_PIXEL_FORMAT_ABGR_8888 || ip->out_fmt == HAL_PIXEL_FORMAT_ARGB_8888) {
        if (infmt == IN_FMT_RGB_888) {
            reg_write(ipu, IPU_OUT_STRIDE, srcw);
        } else {
