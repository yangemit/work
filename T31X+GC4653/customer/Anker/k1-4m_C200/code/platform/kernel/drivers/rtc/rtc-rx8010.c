...
/* TODO: (copy from 8025)Clock precision adjustment */
#define RX8010_ADJ_RESOLUTION	3050 /* in ppb */
#define RX8010_ADJ_DATA_MAX	62
#define RX8010_ADJ_DATA_MIN	-62
...
static int rx8010_get_clock_adjust(struct device *dev, int *adj)
{
	struct i2c_client *client = to_i2c_client(dev);
	u8 digoff;
	int err;

	/*err = rx8010_read_reg(client, RX8010_REG_DIGOFF, &digoff);*/
	if (err)
		return err;
	...
	*adj = digoff >= 64 ? digoff - 128 : digoff;
	if (*adj > 0)
		(*adj)--;
	*adj *= -RX8010_ADJ_RESOLUTION;

	return 0;
}

static int rx8010_set_clock_adjust(struct device *dev, int adj)
{
	struct i2c_client *client = to_i2c_client(dev);
	u8 digoff;
	int err;
	...
	adj /= -RX8010_ADJ_RESOLUTION;
	if (adj > RX8010_ADJ_DATA_MAX)
		adj = RX8010_ADJ_DATA_MAX;
	else if (adj < RX8010_ADJ_DATA_MIN)
		adj = RX8010_ADJ_DATA_MIN;
	else if (adj > 0)
		adj++;
	else if (adj < 0)
		adj += 128;
	...
	/*err = rx8010_write_reg(client, RX8010_REG_DIGOFF, digoff);*/
	if (err)
		return err;
...
static ssize_t rx8010_sysfs_show_clock_adjust(struct device *dev,
					      struct device_attribute *attr,
					      char *buf)
{
	int err, adj;

	err = rx8010_get_clock_adjust(dev, &adj);
	if (err)
		return err;
...
static ssize_t rx8010_sysfs_store_clock_adjust(struct device *dev,
					       struct device_attribute *attr,
					       const char *buf, size_t count)
{
	...
	err = rx8010_set_clock_adjust(dev, adj);

	return err ? err : count;
}

static DEVICE_ATTR(clock_adjust_ppb, S_IRUGO | S_IWUSR,
		   rx8010_sysfs_show_clock_adjust,
		   rx8010_sysfs_store_clock_adjust);

static int rx8010_sysfs_register(struct device *dev)
{
	return device_create_file(dev, &dev_attr_clock_adjust_ppb);
}

static void rx8010_sysfs_unregister(struct device *dev)
{
...
static int rx8010_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	err = rx8010_sysfs_register(&client->dev);

	printk("############## rx8010 register ###############\n");
...
static int rx8010_remove(struct i2c_client *client)
{
	...
	rx8010_sysfs_unregister(&client->dev);
	rtc_device_unregister(rx8010->rtc);
