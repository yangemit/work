...
int sn_contrl_write_bak(HSB_Object_Function *sn_function, void *read_buf)
{
	int i,index,cmd_cur;
	int len_config,len_diff,len_mod;
	int ret = -1;
	...
	sn_function->file_path = UPDATE_UVCCONFIG_BAK_PATH;
...
int sn_burning_cmd(char *read_buf)
{
	...
	case HSB_WRITE_CMEI:
		...
		sn_contrl_write_bak(sn_function, head);
		break;
	...
	case HSB_READ_CMEI:
		...
		if(ret < 0)
			cmd_response(head->cmd, CMD_EXCUTE_ERROR, HID_DATA_INVALID, NULL, 0);
		break;
	case HSB_AUDIO_NOISE_OPEN:
	case HSB_AUDIO_NOISE_CLOSE:
		ret = sample_audio_ns_switch(head->cmd);
		if(ret < 0)
			cmd_response(head->cmd, CMD_EXCUTE_ERROR, HID_DATA_INVALID, NULL, 0);
		else 
			cmd_response(head->cmd, CMD_EXCUTE_OK, HID_DATA_INVALID, NULL, 0);
