...
OUT=$TOP_DIR/out
DATA=$(date +%y%m%d)
T31_FIRMWARE_DIR=$TOP_DIR/firmware/t31
RELEASE_DIR=$(pwd)/release/t31
if [ "$2" != "" ]; then
	TARGET=T31L_firmware_for_USB_Camera_$2_$DATA.bin
else
	TARGET=T31L_firmware_for_USB_Camera_$DATA.bin

fi

# dd if=$T31_FIRMWARE_DIR/uboot-t31L-Nodelay of=$OUT/$TARGET &&
dd if=$T31_FIRMWARE_DIR/iboot.bin of=$OUT/$TARGET &&
dd if=$T31_FIRMWARE_DIR/uImage-with-cmdline of=$OUT/$TARGET bs=1k seek=32 &&
dd if=$T31_FIRMWARE_DIR/rootfs_for_ucamera of=$OUT/$TARGET bs=1k seek=2080 &&
dd if=$OUT/appfs.jffs2 of=$OUT/$TARGET bs=1k seek=4128
