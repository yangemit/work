...
if [ "$2" != "" ]; then
	TARGET=T31A_firmware_for_USB_Camera_$2_$DATA.bin
else
	TARGET=T31A_firmware_for_USB_Camera_$DATA.bin

fi

dd if=$T31_FIRMWARE_DIR/uboot-t31A-16m-0101 of=$OUT/$TARGET &&
dd if=$T31_FIRMWARE_DIR/uImage of=$OUT/$TARGET bs=1k seek=256 &&
