...
#define IAAC_VERSION_INFO "tseries-2.0.0"
...
typedef struct {
	...
	char                 ip[64];								/**< 远端服务的IP，若ip的长度为0，则可直接使用ip，否则使用url */
	int		        port;								/**< 远端服务的端口号 */
...
/**
 * @fn char *IAAC_GetVersionInfo(void)
 *
 * 获取IAAC库版本信息
 *
 * @retval IAAC库版本信息
 *
 * @remarks 用于获取IAAC库版本信息，包括芯片系列，IAAC库协议版本
 */
char *IAAC_GetVersionInfo(void);
