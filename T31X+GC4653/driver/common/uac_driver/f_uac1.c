...
static int in_sample_rate = MIC_SIMPLE_RATE;
module_param(in_sample_rate, int, S_IRUGO);
MODULE_PARM_DESC(in_sample_rate, "dmic sample_rate");

static int mic_channel = MIC_CH;
module_param(mic_channel, int, S_IRUGO);
MODULE_PARM_DESC(mic_channel, "mic channel");

static int in_ep_actual_size = (MIC_SIMPLE_RATE * MIC_CH * 2 * 40 / 1000); //40ms
static int in_ep_req_count = (MIC_CH * 10); //800ms
...
static int in_ep_max_packet_size = (MIC_SIMPLE_RATE * MIC_CH * 2 /1000);
...
static unsigned int req_data_cnt = 0;
/*************************************************************/
/*
		...
		const struct usb_ctrlrequest *ctrl)
{
	...
	printk( "!!!!set: bRequest 0x%x, w_value 0x%04x, len %d, entity %d\n",
		...
		const struct usb_ctrlrequest *ctrl)
{
	...
	struct usb_audio_control_selector *cs;
	struct usb_audio_control *con;
	printk("@@@@get: bRequest 0x%x, w_value 0x%04x, len %d, entity %d\n",
			ctrl->bRequest, w_value, len, id);
		...
		if (req) {
			...
			if (req->buf) {
				...
				if (err)
					...
					"%s queue req: %d\n",
					out_ep->name, err);
				audio_req_put(audio,&audio->play_reqs,req);
			} else
				err = -ENOMEM;
		} else
			err = -ENOMEM;
...
static void spk_interface_disconn(struct usb_function *f)
{
	...
	{
		if(!list_empty(&audio->play_reqs))
		{
			if(audio)
				req = audio_req_get(audio, &audio->play_reqs);
			kfree(req->buf);
...
#ifdef CONFIG_GADGET_UAC1_CAP
	/* Set channel numbers */
	mic_input_terminal_desc.bNrChannels = mic_channel;
	mic_as_type_i_desc.bNrChannels = mic_channel;
	as_in_ep_desc.wMaxPacketSize = __constant_cpu_to_le16(in_ep_max_packet_size);
	...
	while (r > 0) {
		...
		if (req) {
			...
			if (req_data_cnt < 2) {
				...
				req_data_cnt++;
			} else if (!audio_is_req_empty(audio,&audio->audio_data_list) && first_queue) {
				/* trigger */
				first_queue = 0;
			...
			req_null_cnt++;
			if (req_null_cnt > 5) {
		//		printk("################### %d, %d ###################", req_null_cnt, req_data_cnt);
				audio->cur_req = NULL;
...
#error cap or paly should be defined
	...
	{
		struct usb_request *req = audio_request_new(audio->in_ep, in_ep_actual_size);
		if(req){
			req->context = audio;
...
int  control_selector_init(struct f_audio *audio)
{
	...
	if (speak_enable) {
		...
		spk_volume_control.data[UAC__MIN] = 0x0000;
		spk_volume_control.data[UAC__MAX] = 0x0064;
		spk_volume_control.data[UAC__RES] = 0x0001;
		/*
		 *spk_volume_control.data[UAC__CUR] = 0x0000;
	...
	mic_volume_control.data[UAC__MAX] = 0x006e;
...
int audio_bind_config(struct usb_configuration *c)
{
	...
	if (speak_enable)
		...
		INIT_LIST_HEAD(&audio->play_reqs);
#endif
...
int uac_set_descriptor(unsigned long arg)
{
	...
	in_ep_actual_size = (in_sample_rate *mic_channel* 2  * 40 / 1000);
	in_ep_max_packet_size = (in_sample_rate * mic_channel * 2 / 1000);
	if(in_sample_rate == 44100)
		in_ep_max_packet_size += 4;

	in_ep_req_count = (1 * 20);
