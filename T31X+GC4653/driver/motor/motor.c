...
#define MOTOR_CNT                1
#define VERTICAL_MOTOR_ENABLE    0
...
static irqreturn_t jz_timer_interrupt(int irq, void *dev_id)
{
	...
	if (islimit) {
		...
		if (motors[HORIZONTAL_MOTOR].cur_steps >= motors[HORIZONTAL_MOTOR].max_steps) {
			if (motors[HORIZONTAL_MOTOR].state == MOTOR_OPS_RESET) {
				motors[HORIZONTAL_MOTOR].state = MOTOR_OPS_STOP;
				motors[HORIZONTAL_MOTOR].max_steps=hmaxstep;
				complete(&motors[HORIZONTAL_MOTOR].reset_completion);
				/* motors[HORIZONTAL_MOTOR].move_dir = MOTOR_MOVE_LEFT_DOWN; */
			}
		}
...
static long motor_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
		...
		case MOTOR_PWR:
			{
				unsigned long motor_pwr = 0;
				if (copy_from_user(&motor_pwr, (void __user *)arg, sizeof(unsigned long))) {
					dev_err(mdev->dev, "[%s][%d] copy from user error\n",
						__func__, __LINE__);
					return -EFAULT;
				}
				...
				if(motor_pwr == 1){
					gpio_direction_output(GPIO_MOTOR_PWR, MOTOR_ENABLE);
					mdelay(20);
				}else if(motor_pwr == 0){
					gpio_direction_output(GPIO_MOTOR_PWR, MOTOR_DISENABLE);
					mdelay(20);
				}
				/*printk("MOTOR_PWR!!!!!!!!!!!!!!!!!!!\n");*/
				break;
			}
		case MOTOR_SET_PARAMS:
			{
				if(arg == NULL){
					ret = -EPERM;
					break;
				...
				if (copy_from_user(&motor_params, (void __user *)arg, sizeof(motor_set_params_t))) {
					printk("ERROR(%s): MOTOR_SET_PARAMS error!\n", __func__);
					return -EFAULT;
				}

				/* set hmaxstep, vmaxstep, islimit */
				hmaxstep = motor_params.hmaxstep;
				mdev->motors[HORIZONTAL_MOTOR].max_steps = hmaxstep;
#if VERTICAL_MOTOR_ENABLE
				vmaxstep = motor_params.vmaxstep;
				mdev->motors[VERTICAL_MOTOR].max_steps = vmaxstep+30;
#endif
				islimit = motor_params.islimit;
...
static int motor_probe(struct platform_device *pdev)
{
	...
	/* motor power gpio */
	if(GPIO_MOTOR_PWR != -1){
		gpio_request(GPIO_MOTOR_PWR, "GPIO_MOTOR_PWR");
	}
