...
#  endif
#else
#  include "include/android_reboot.h"
#endif
...
void *adb_bootstrap_func(void *x)
{
    stinfo *sti = x;
    sti->func(&sti->fd, sti->cookie);
    return 0;
...
static void subproc_waiter_service(int* fd, void *cookie)
{
    ...
    if(name) {
        *fd = create_subprocess(SHELL_COMMAND, "-c", name, &pid);
    } else {
        *fd = create_subprocess(SHELL_COMMAND, "-l", 0, &pid);
    }
...
static int create_subproc_thread(const char *name)
{
    ...
    sti->cookie = (void*)name;
    sti->fd = -1;

    if(adb_thread_create( &t, adb_bootstrap_func, sti)){
        fd = -1;
        goto out;
    }
